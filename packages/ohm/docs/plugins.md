...
An ohm plugin is a package of code, skills, prompts, or themes. Code default-exports one trusted in-process factory receiving `PluginAPI` from `ohm/plugins`. Commands, tools, event handlers, providers, services, renderers, flags, and shortcuts registered during activation become visible only after activation succeeds. No wrapper or second registration system is needed.

For package layout, installation, integrity, and publishing, read [Plugin packages](packages.md). Choose a focused package from the [examples catalog](../examples/README.md); the smallest runnable package is [`examples/starter`](../examples/starter/README.md).

The same factory can grow into a useful optional feature: [workspace memory](../examples/workspace-memory/README.md)
uses atomic configuration, a context hook, and portable remember/forget actions;
[code review](../examples/code-review/README.md) owns a public RPC client,
cancellation, and a saved session for explicit continuation. Start with the
smallest registration you need: commands, tools, event listeners, and `onDispose`
cover ordinary authoring. Host-specific lifecycle composition is an advanced
capability, not a second kind of plugin.

Create a working copy with `ohm plugins init ./my-plugin`.
Use `ohm plugins test PACKAGE` (or its `npm test`) for behavior checks and
`ohm plugins verify PACKAGE` for source and installed-archive checks.
During development, `ohm plugins preview PACKAGE` opens a reviewed local
copy in the normal host without installing it or saving the session. Use `/actions`
for portable actions and `/refresh` after editing. This is trusted code, not a sandbox.

Use `--no-plugins` to suppress automatic discovery of all plugin resources, or
`--no-plugin-code` to suppress only executable code and retain skills, prompts,
and themes. Explicit `--plugin PATH` selections still load in either case.

## One package, one runtime API

| Surface | Role |
| --- | --- |
| Package | Distribution envelope. A native `package.json` can declare plugin factories, skills, prompts, and themes together. |
| Plugin code | Trusted executable entry. Every factory receives the same generation-scoped `PluginAPI`. |
...
The [Plugin API reference](plugin-api.md#top-level-methods) documents registrations,
session actions, configuration, services, processes, and discovery on the one
factory object. Start with commands, tools, event handlers, and `onDispose`;
use the other capabilities when the workflow needs them.

The API object is generation-scoped. Do not cache it across refreshes.

Raw terminal components stay TUI-only. For session-owned portable JSON views,
shared state, and versioned services, see the advanced
[host lifecycle and presentation contracts](facets-and-presentations.md). These
remain capabilities of the same factory API; ordinary plugins do not need them.

Registrations remain live after activation. Re-registering a tool, its renderer, or a command with the same name from the same plugin replaces that registration atomically. A tool name already owned by another plugin keeps its first owner and produces a diagnostic.

Every `register*` method, `on`, `onDispose`, and `events.on` returns the same
callable `PluginRegistrationHandle`. Call the handle or its `dispose()`
...
- plugin-owned `paths.userData` and canonical-workspace-isolated `paths.workspaceData`;
...
A plugin command runs immediately and does not pass through `input`
...
The host creates isolated data roots for each plugin owner:
...
Use fixed filenames or validate them at the boundary. Schema-check and bound every read, use user-only file permissions, and define a concurrency strategy. Credentials belong in the host credential store, never plugin data. See [`state-and-policy`](../examples/state-and-policy/README.md).
...
`prepareArguments(raw)` receives an isolated, bounded JSON snapshot before the
complete parameter schema is applied. Use it for deterministic normalization,
such as decoding a stringified argument envelope. Do not perform side effects
there: the prepared result must still pass schema validation, tool-call hooks,
resource claims, and authorization before `execute` runs.

`renderCall` may run while arguments are still streaming. Its input is
recursively partial, so check nested fields and array elements before using
them. `context.argsComplete` means the model finished supplying arguments, not
that execution validation succeeded. Rendering does not relax tool validation.
...
Unsupported grammar routes use the ordinary function schema. Do not assume a constraint was enforced unless the selected model advertises the matching capability. See the [tool contract](plugin-api.md#tool-contract).
...
An MCP bridge is an ordinary trusted plugin. It owns the transport,
...
Specialist delegation is still a plugin-owned workflow. The package owns
...
Plugins create SDK sessions or use the public `RpcClient` directly and own
their lifecycle, as the [`code-review` example](../examples/code-review/README.md)
does. `ohm.processes` is available for generation-owned subprocesses. The host
does not expose a child-session service or own delegation transports or journals.

The optional generic `ohm.jobs` service records bounded workflow metadata and
results, cancellation, and explicit recovery. It does not launch or reconnect
agents. All job calls are post-commit operations, including inspection and
listing, because reading the registry may persist crash recovery. Factories
should register the workflow and begin it from a later callback.

The [`subagent-specialists` example](../examples/subagent-specialists/README.md)
shows managed-process delegation. Any restart and reattachment policy belongs
to the plugin.
...
- provider request hooks and complete request/response header hooks for trusted direct plugins;
...
snapshot boundary before a plugin handler runs. The 1,024-emission and 4 MiB
aggregate limits apply only to the activation queue.

`ohm.services` serves a different trusted-only use case: cooperating plugins
...
Direct plugins use the public `@ohm/models` `Model`, `Provider`, `Api`, and streaming contracts. A provider may declare a custom API identifier when it also supplies `streamSimple`; the host preserves that identifier in plugin contexts and translates it explicitly at the core run-loop boundary.
...
Callback contexts expose the current session through a read-only `SessionManager` projection. It includes the session header, append-only entries, tree, labels, branch context, session name, file path, and leaf. Read through this projection instead of reopening the database. Session text is private and untrusted.
...
plugin work in `context.paths` and retry after a matching `session_start`
rather than retaining an implicit-current factory sender.

Pair `appendEntry(customType, data)` with `registerEntryRenderer(customType, renderer)`, and pair visible `sendMessage` calls with `registerMessageRenderer`. These renderers project the durable entry directly: the same stored value, ID, and order are used during live display and resume. Hidden messages stay hidden, and a renderer failure uses the host fallback without changing the session.

Path-loaded plugin entries and messages expose host-owned provenance through
the read-only session projection and durable replay. The versioned envelope
identifies the exact plugin ID and loaded source digest. It includes package
...
The command context owns interactive UI. Simple plugins should use notifications, dialogs, text widgets, status, working messages, and editor text helpers. Trusted TUI plugins may import public components from `ohm/tui` and install a complete editor factory, autocomplete wrapper, header, footer, widget, custom overlay, or bounded named route.

`context.ui.setStatus` contributes compact keyed text to one shared footer status row. It does not create an independently placed content row. Use `context.ui.setWidget` when the plugin needs a dedicated block above or below the editor.
...
The declared plugin ID remains display metadata. ohm keys UI ownership by the full contribution source and generation, so concurrently active sources with the same declared ID retain independent status, working state, components, and cleanup.
...
Use `ohm.processes.spawn({ argv, ... })` for long-lived or concurrently observed workers. The host owns generation and host concurrency caps, bounded capture or backpressured pipes, cancellation, and process-tree cleanup. The plugin still owns its protocol, validation, task-level concurrency, recursion policy, failure presentation, and result delivery. Workers start only after activation commits, end on refresh or close, and never reattach after a host restart.

Use the [managed-process contract](plugin-api.md#managed-asynchronous-processes) for a framed worker or plugin-owned delegated agent. Use an [external execution backend](execution-backends.md) when a model tool must cross a reviewed isolation boundary. A process launched by a plugin is not a sandbox merely because it runs another agent session.
...
Relative paths resolve from the package root and must stay inside it, including
after resolving symlinks. Trusted plugins may return explicit absolute paths
to select resources in the workspace, shared directories, or their owned data
directories. Selected paths retain canonical regular-file checks, bounded reads
and directory traversal, and normal collision diagnostics. Untrusted project
plugins cannot contribute resources. Trusted code owns this path selection;
resource discovery is not a sandbox and does not widen model tools' filesystem
access. See [`dynamic-package`](../examples/dynamic-package/README.md).
