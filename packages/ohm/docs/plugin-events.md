...
Direct factories subscribe with `ohm.on(name, handler)`. The names and payloads below are the public contract exported from `ohm/plugins`. Internal durable-event envelopes and provider-native stream objects are not plugin APIs.
...
| `input` | Continue, transform, or consume accepted interactive/RPC/serve/plugin input. | `continue`, `transform`, or `handled` result |
...
Provider header hooks belong to the trusted direct-plugin tier. They can observe authentication and cookie headers, just as installed in-process code can already inspect process memory and environment variables. Bounded core diagnostics and session exports remain allowlisted and redacted.
...
const subscription = ohm.events.on("index-ready", (value) => {
  // consume bounded plugin-owned data
});
...
validated again before the plugin handler runs. The queue-count and aggregate
