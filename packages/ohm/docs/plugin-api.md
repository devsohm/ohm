...
A plugin's code is one trusted in-process factory:
...
`ohm/plugins` exports the complete authoring API. All capabilities below belong
to that one factory.
...
![Plugin generation lifecycle and capability flow](assets/plugin-lifecycle.svg)
...
`onDispose(callback)` registers plugin-owned cleanup. Cleanup callbacks:

- run once in reverse registration order;
- run after the generation API has become stale;
- continue after one callback fails;
- may be asynchronous;
- must not call the stale `PluginAPI`.

Use disposal for resources that the plugin creates. These resources include timers, watchers, sockets, raw child processes, and temporary files. The host removes commands, tools, listeners, services, providers, renderers, shortcuts, flags, UI mounts, and processes started through `ohm.processes`.
...
The focused `ohm/plugins` entrypoint exports the named authoring contracts
...
`PluginEventMap` and `PluginEventResultMap`; and tool resource callbacks
...
modules are declaration implementation details, not package subpaths; plugin
authors use only the composed names exported by `ohm/plugins`.

## Top-level methods

The complete `PluginAPI` surface is:

| Member | Return | Contract |
| --- | --- | --- |
| `onDispose(callback)` | `PluginRegistrationHandle` | Register generation cleanup. |
| `on(event, handler)` | `PluginRegistrationHandle` | Register one typed event listener. |
| `registerTool(tool)` | `PluginRegistrationHandle` | Register or replace this plugin's tool by name. |
| `registerCommand(name, options)` | `PluginRegistrationHandle` | Register a slash command. |
| `registerShortcut(shortcut, options)` | `PluginRegistrationHandle` | Register a TUI keybinding. |
| `registerFlag(name, options)` | `PluginRegistrationHandle` | Declare a boolean or string CLI flag. |
| `getFlag(name)` | `boolean \| string \| undefined` | Read the parsed value or declared default. |
| `registerMessageRenderer(customType, renderer)` | `PluginRegistrationHandle` | Render live and resumed custom messages. |
| `registerMarkdownTransformer(transformer)` | `PluginRegistrationHandle` | Transform ordinary transcript Markdown for display only. |
| `registerEntryRenderer(customType, renderer)` | `PluginRegistrationHandle` | Render durable custom session entries. |
...
| `config` | `PluginConfigStore` | Read or compare-and-swap the plugin's user or workspace configuration. |
| `jobs` | `PluginJobService` | Start, inspect, list, cancel, wait for, and explicitly resume durable plugin jobs. |
| `processes` | `PluginProcessService` | Start and control generation-owned asynchronous processes. |
| `services.register(name, service)` | `PluginRegistrationHandle` | Publish one trusted same-process object or function by exact reference. |
...
| `getCommands()` | `SlashCommandInfo[]` | Unified invokable plugin-command, prompt-template, and skill-command catalog. |
...
| `registerProvider(provider)` | `PluginRegistrationHandle` | Install a complete `@ohm/models` provider for this generation. |
| `registerProvider(id, config)` | `PluginRegistrationHandle` | Compose a provider configuration. |
| `unregisterProvider(id)` | `void` | Remove this generation's provider registration early. |
| `events.on(channel, handler)` | `PluginRegistrationHandle` | Subscribe to the process-local plugin topic bus. |
| `events.emit(channel, data)` | `void` | Emit on that topic bus. |

Each `getAllTools()` entry contains `sourceInfo`. Plugin tools keep their resolved package path, source, scope, and
origin. Built-in and caller-owned tools use temporary synthetic sources named `<builtin:name>` and `<host:name>`.

`PluginRegistrationHandle` is callable and also exposes `dispose()` plus a
...
`ohm.services` is the narrow escape hatch for two trusted plugins that need
...
plugins cannot see it. A failed activation publishes none of its services.
...
### Sharing tools and plugin capabilities

Plugins loaded into the same session contribute to one tool catalog. Use
`getAllTools()` to inspect built-in, caller-owned, and plugin tool metadata, and
`setActiveTools()` to select which tools the model can use. These methods do not
return executable callbacks. There is no general `callTool(name, input)` method
on `PluginAPI`; ordinary model tool calls run through the session's coordinator,
authorization, resource claims, and journal.

For two plugins to reuse the same behavior, publish a deliberate service or
share an ordinary module. Use the topic bus for notifications, not a hidden
request/response protocol. Private state, providers, UI components, and callbacks
are not automatically inherited. A service call is ordinary trusted JavaScript,
not a journaled tool invocation or a permission boundary.

A separately created SDK session or process has its own runtime and lifecycle.
Compose its tools and providers explicitly using the public SDK and fresh
factories. Loading the same package there creates another plugin instance; it
does not transfer the original instance's state. Do not retain a parent
generation's callbacks as a substitute for activating the plugin in that host.
...
For path-loaded plugins, the host records an optional immutable `provenance`
...
values are known. Plugins do not supply this identity. Loose files never
...
`{ expandPromptTemplates: true }` to opt into the host's plugin-command,
...
Top-level send methods schedule asynchronous delivery and return immediately. They report neither admission nor model completion. Delivery failures become plugin diagnostics.
...
Use `ohm.processes` when the plugin must return immediately, report status,
...
`ohm.jobs` supplies durable host authority without choosing a plugin's
...
Every `jobs` method is available only after the plugin
factory commits, including read methods such as `inspect`, `list`, and `wait`.
Reads can perform crash recovery and persist an ownership transition,
...
Current bounds are eight active jobs and 256 retained records per plugin,
...
segments (`[a-z][a-z0-9._-]*`); the `ohm.` prefix is reserved. Retained legacy
records with that prefix remain inspectable and cancellable, but cannot be
resumed as generic jobs. Because retention is bounded, a sufficiently old terminal or interrupted ID and its
...
plugin's private workspace data directory. Ownership is stable across
generations of the same plugin, while another plugin cannot inspect or
...
### Plugin-owned protocol bridges and delegated workflows

There is no MCP-specific registry or built-in subagent workflow in this API.
Both integrations remain ordinary trusted plugins. A delegation plugin owns
its SDK sessions or RPC clients and their cleanup and recovery. Generic `jobs`
can persist workflow metadata and results; it does not manage agent sessions.
...
plugin replace or retire its own tool set without a protocol-aware core API.
The tested [`mcp-stdio` example](../examples/mcp-stdio/README.md) implements that
complete boundary.

A specialist-delegation plugin owns its profiles, orchestration, limits,
result composition, and presentation. It can launch a one-shot JSON-mode process
through `ohm.processes`, or manage a steerable session through the public SDK
or `RpcClient`, and expose the workflow as one ordinary registered tool.
...
Ordinary event handlers, tools, and shortcuts receive `PluginContext`:
...
| `paths.userData` | Plugin-owned durable cross-workspace directory |
| `paths.workspaceData` | Plugin-owned durable directory isolated to the canonical workspace |
...
never follow a later host rebind. Calls reject after the plugin generation or
captured session becomes stale. This is a live-session acknowledgement, not a
durable cross-session mailbox: plugin-owned background work should persist
...
### Host-managed plugin configuration
...
main `config.json` settings system. `ohm/plugins` also exports
`createPluginConfigStore()` for embedding hosts that construct the same
primitive explicitly.

Every replacement or removal requires the `revision` returned by `read()`.
Conflicting writers receive `PluginConfigConflictError` instead of silently
...
type PluginConfigScope = "user" | "workspace";
...
interface PluginConfigReadOptions {
  readonly signal?: AbortSignal;
}

interface PluginConfigWriteOptions extends PluginConfigReadOptions {
...
interface PluginConfigStore {
  read(
    ...
    options?: PluginConfigReadOptions,
...
present value. Validate and bound `snapshot.value` against the plugin's own
...
```ts
import type { JsonValue } from "ohm";
import { PluginConfigConflictError, type PluginAPI } from "ohm/plugins";
...
async function removeWorkspaceConfig(ohm: PluginAPI, signal: AbortSignal) {
  const current = await ohm.config.read("workspace", { signal });
...
the same declared plugin ID but different package-relative source entries do
...
the user and workspace documents, interpret a plugin schema, or provide a
credential vault. Plugin configuration values are never diagnostics or
telemetry fields.

Command handlers receive `PluginCommandContext`, which adds:
...
`setup` receives the new mutable `PluginSessionManager` before it is exposed. `withSession` receives a frozen `ReplacedSessionContext` after rebinding. That context adds asynchronous `sendMessage` and `sendUserMessage` methods so follow-up work targets the replacement rather than the stale session.
...
session ID. Its plugin-command `newSession`, `fork`, and `switchSession`
...
```ts
interface ReadonlyPluginSessionManager {
  getCwd(): string;
...
Returned entries are the public plugin projection. One canonical tool batch can project as multiple public entries with derived IDs. Read this view instead of reopening and racing the active session database.
...
above are public imports from `ohm/plugins`.

### Model registry

`PluginModelRegistry` exposes:
...
after its plugin generation or session binding becomes stale. This is one
...
plugin code; never place their values in output, session data, or errors.
Provider registration details are in [Provider authoring](provider-authoring.md).

## UI context

`hasUI` says whether dialogs can be requested. `ui.capabilities` negotiates the exact available surfaces. A missing capability map or a false member means unsupported, which keeps older custom hosts source-compatible. A plugin must keep a safe headless path.

| Method or property | Contract |
| --- | --- |
| `capabilities?` | Frozen `PluginUICapabilities` map for exact host feature negotiation. |
...
Only one plugin route can be active in the rich TUI. Opening another closes
and disposes the previous route, even when another plugin owns it. The route
...
registration owned by that plugin generation. The four stable paths are
`session.header`, `session.beforeEditor`, `session.afterEditor`, and
`session.footer`. `PLUGIN_UI_SLOT_PATHS` and the slot types are public from
`ohm` and `ohm/plugins`.
...
host plugin-load encounter, then registration encounter. The greatest
...
| --- | --- | --- |
| Rich TUI | `tui` / `true` | Full dialogs, notifications, keyed status, one active plugin route, ordered session slots, themes, editor state, and terminal components. |
...
| RPC | `rpc` / `true` | Dialogs, notifications, status, string-array widgets, title, whole-draft text replacement, and cursor-relative paste requests cross the typed RPC UI bridge. Plugin routes, session slots, raw terminal components, and theme switching are unavailable. |
...
SDK and embedding hosts report the first-class `sdk` `PluginMode` with
...
`custom` options are `{ overlay?, overlayOptions?, onHandle? }`. `overlayOptions` may be an object or a function, and `onHandle` receives the live `OverlayHandle`. The plugin must arrange for `done` to run or for cancellation/disposal to settle the operation.
...
```ts
{
  ...
  handler(args: string, context: PluginCommandContext): Promise<void>;
}
```
...
`registerShortcut(shortcut, { description?, handler })` uses a public `KeyId`. Shortcuts are interactive-only but their handler context remains the ordinary plugin context.
...
Active transformers run in plugin order for ordinary user text, assistant text, and visible reasoning content.
...
```ts
import { defineTool } from "ohm/plugins";
...
| `prepareArguments?(raw)` | Normalize an isolated, bounded JSON snapshot before complete parameter-schema validation; no side effects. |
...
| `renderCall?`, `renderResult?` | Optional TUI components. Call arguments are recursively partial while streaming; guard missing fields even when `argsComplete` is true because rendering precedes execution validation. |
...
File-mutating plugin tools must import `withFileMutationQueue` from `ohm`
...
```ts
(event, context: PluginContext) => result | void | Promise<result | void>
```
...
Context messages carry an optional opaque `contextId`. Preserve it when filtering,
reordering, or editing messages, including through object spread or JSON cloning.
It identifies an input message for that context roundtrip, not a session-entry ID;
do not invent, duplicate, persist, or reuse it in a later callback. Each projected
tool result has its own identity, even when several came from one tool batch.

A message without `contextId` is new and cannot introduce provider continuation
state or response metadata. With an identity, the message role and provider-owned
fields remain protected. Omitting continuation state retains the original state;
changing it, its provider/API/model source, or response metadata is rejected. To
replace an assistant with a new state-free message, omit both the identity and all
provider-owned fields. Context edits affect the next model input, not the journal.
...
terminal UI is present. RPC, serve, and plugin-originated prompts use their
...
- Listeners run in deterministic plugin-load and registration order.
...
- Dispatch cancellation owns only the in-flight callback. Persistent UI from a
  successfully settled listener remains owned by its plugin generation until
  the plugin clears it or that generation ends.
...
Use `ohm/plugins` for this API, public tool claims and recovery types; `ohm/tui` for public terminal components; `ohm/providers` for provider transport types; and `ohm/storage` for public session types. Do not import `src/`, `dist/`, or private module paths, and do not bundle another copy of the host runtime into a plugin package.
