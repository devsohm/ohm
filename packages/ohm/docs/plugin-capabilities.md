...
The machine-readable [`plugin-capabilities.json`](plugin-capabilities.json)
maps each supported plugin capability to its hosts, public API members,
...
| `task-prompts` | Declarative prompt templates without a runtime factory |
...
| `subagent-specialists` | Plugin-owned named delegation through ordinary tools and managed processes |
| `mcp-stdio` | Plugin-owned MCP discovery and tool registration through a managed stdio process |
| `dynamic-package` | Runtime-discovered skills and prompts |
| `provider-hooks` | Request mutation and complete request/response header observation for trusted direct plugins |
...
| `state-and-policy` | Plugin-owned workspace memory, task state, and protected-path preflight |
...
Named plugin routes are a rich-TUI-only structured component surface. One
...
agents remain trusted plugin code; untrusted packages must not be loaded as
direct plugins.

Durable jobs add host-owned identity, atomic bounded metadata, cancellation,
and explicit recovery without moving workflow policy into the host. A restarted
host reports prior live work as interrupted and requires an explicit new job
operation; it never claims that a callback or process survived. A second live
host cannot take ownership from the first. Plugins own delegated sessions and
their transports through the public SDK or RPC client; there is no host-owned
...
The HTTP service keeps session identity under its endpoint registry. Plugin
