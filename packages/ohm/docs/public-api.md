...
| Advanced stable | `ohm/auth`, `ohm/config`, `ohm/context`, `ohm/core`, `ohm/images`, `ohm/net`, `ohm/plugins`, `ohm/process`, `ohm/prompts`, `ohm/providers`, `ohm/service`, `ohm/serve`, `ohm/storage`, `ohm/tools`, `ohm/tui` | Low-level composition. These APIs can carry more host authority and require the caller to own lifecycle and invariants. |
...
Use `ohm/plugins` for plugin authoring. Its factory, resources, and optional
host capabilities form one system.

`ohm/serve` exposes `createServeSessionRuntime(() => session, lifecycle?)` to
adapt the canonical session for HTTP without private imports. The host owns
startup, shared-service cleanup and subscription rebinding when it replaces a
session. See [HTTP host ownership](serve.md#embed-the-http-host) and the
[offline client proof](../examples/serve-headless.md).
...
The complete packaged export inventory is [`release/public-subpaths.json`](https://github.com/devsohm/ohm/blob/main/packages/ohm/release/public-subpaths.json), and the release check requires it to exactly match `package.json`.

The entry points above serve ordinary consumers and managed plugin runtime entries. Runtime plugins cannot value-import:
...
ohm resolves these specifiers to the running host. This keeps one canonical host instance when an installed plugin lives outside ohm's dependency tree. Type-only authoring imports remain governed by TypeScript and the package export map.
