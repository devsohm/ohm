...
interface AuthoringTask {
  ...
  checks: string[];
  sources: string[];
  verify(host: RuntimePluginHost, resources: ResolvedPaths): void | Promise<void>;
}
...
async function verifyHost(task: AuthoringTask, host: RuntimePluginHost, resources: ResolvedPaths): Promise<void> {
...
async function activateCandidate(candidate: PackageActivationCandidate): Promise<void> {
  ...
  const options: Parameters<typeof loadDirectPlugins>[1] = {
  ...
  const host = await loadDirectPlugins(selected.paths, options);
  await host.close();
}
...
async function verifyAttempt(
  ...
  let installed = false;
  let firstHost: RuntimePluginHost | undefined;
  let secondHost: RuntimePluginHost | undefined;
  try {
    ...
    stage = "activation";
    firstHost = await loadDirectPlugins(direct.paths, {
      workspace,
      activationFailure: "throw",
    ...
    secondHost = await loadDirectPlugins(refreshed.paths, {
      workspace,
      activationFailure: "throw",
