...
The versioned JSON report conforms to [`runtime-performance-report.schema.json`](runtime-performance-report.schema.json). Its eleven scenarios cover cold startup with 0, 10, and 50 runtime plugins. They also cover a refresh with eight runtime entries and 256 commands. The suite reopens durable sessions and projects conversation context from 100 and 10,000 message events. It then replays those histories through the cursor-paged RPC subscription path. Three cold-page fixtures request one event from linear histories containing 10,000, 16,002, and 20,000 events. Each fixture counts canonical rows returned by single-entry, full, and paged manager reads and enforces a 16-row budget. These counters do not observe internal SQLite payload reads; separate storage regressions check those backing reads. The large RPC replay uses one event per page. It detects replay work whose cost grows with the complete remaining history instead of the requested cursor page. Every replay wait has a scenario ceiling, so a stalled cursor fails locally instead of consuming the outer CI timeout. Fixture creation and cleanup are outside the reported intervals.
...
[`risk-coverage.config.json`](risk-coverage.config.json) keeps independent line, branch, and function baselines for plugin activation, CLI orchestration, TUI state, service orchestration, durable storage, and the HTTP/SSE transport. Five canonical test groups measure the plugin runtime, CLI/TUI, service runtime, session storage, and serve transport targets through c8's subprocess-aware Monocart reporter. The gate reads Monocart's native V8 line, branch, and function percentages directly. It does not synthesize untouched sources or convert the results through Istanbul/LCOV. The session-storage group uses the storage and service behavior suites. Both the session manager and its shared message-projection helper retain the storage baselines. This scope measures the storage API and its product consumer without merging unrelated CLI, loader, and PTY process identities into the same source map.
