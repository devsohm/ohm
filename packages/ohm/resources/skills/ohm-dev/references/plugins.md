...
Use one plugin package for code, skills, prompts, and themes. New TypeScript code
imports `PluginAPI` from `ohm/plugins` and default-exports one factory. Start with
commands, tools, events, and `onDispose`; advanced host lifecycle parts are not a
second authoring method. Create with `ohm plugins init DIRECTORY`, then use
`ohm plugins test`, `preview`, and `verify`. Declare code with `ohm.entrypoints`
and resource paths with `ohm.skills`, `ohm.prompts`, and `ohm.themes` in the same
`package.json`; do not add another manifest, loader, or plugin store.

## Read the installed contract first

Resolve every relative link from this file.

1. Read [the plugin overview](../../../../docs/plugins.md) completely.
...
3. Read the complete [API reference](../../../../docs/plugin-api.md) when a signature or lifecycle detail is not shown by the focused example. Read [plugin events](../../../../docs/plugin-events.md) for event work. For behavior across hosts, use the [capability matrix](../../../../docs/plugin-capabilities.md) and [mode contract](../../../../docs/modes.md); also read [RPC](../../../../docs/rpc.md) when an interaction must cross that protocol.
...
7. Inspect the smallest matching example completely: its `package.json`, factory, README, any example-local tests, and referenced resources. In a source checkout, inspect the nearest central conformance test when the example has no local test; do not copy its private imports into a plugin package. Installed artifacts do not ship host source or central tests, so use their public declarations, examples, author checks, and real-loader smoke instead.
...
| Reusable review, diagnosis, or implementation instructions | `../../../../examples/task-prompts/` |
...
The plugin entry default-exports its activation factory:
...
Use `ohm/plugins` for types and only documented public host subpaths at runtime. Put runtime libraries in `dependencies`, build/test tools in `devDependencies`, and ohm itself in peer and development dependencies when declarations are needed. Never import `src/`, `dist/`, or another bundled ohm runtime.

## Lifecycle and authority

- Keep activation deterministic and fast. Start long-lived work lazily unless it must exist immediately.
- Register every raw plugin-owned timer, watcher, socket, server, subprocess, and temporary resource with `ohm.onDispose`. Host registrations and `ohm.processes` workers are generation-owned.
- Disposers run after the API becomes stale. Release only captured plugin-owned resources; do not call ohm APIs from a disposer.
...
- Use `ohm.processes` for external protocol workers and delegated-agent processes. The plugin owns admission, concurrency, recursion, cancellation policy, framing, validation, result composition, and bounds; the host owns only generic generation cancellation and process-tree cleanup.
...
- Validate and bound command arguments. Use `context.sessionManager`; never reopen the active session file or parse it as text. Saved sessions use SQLite; JSONL is a transfer format.
...
- Never interpolate untrusted input into a shell command. Await or cancel raw child processes and stop them during disposal. Managed workers end automatically when their generation refreshes or closes, but the plugin still owns protocol validation, task concurrency, and result delivery.
...
3. Run `ohm plugins report PACKAGE_DIRECTORY` through the real direct loader. Author reports accept real local package directories, not `.tgz` files.
...
8. Pack to an explicit destination directory outside the package root. Use the JSON `artifact` path and SHA-256 as the authority; `ohm plugins pack` validates those exact archive bytes, so a second report on extracted bytes is not required.
...
10. Use `ohm --plugin PATH` only for an invocation-only source smoke.
...
Report the package root, entry point, visible behavior, authority, focused tests, install/refresh/remove evidence, and any compatibility or publication caveat. Do not call a plugin production-ready without installed-artifact evidence.
