...
import type { PluginWireServiceDescriptor } from "../../src/plugins/wire-services.js";

type BrokerAgentSession = Pick<
  AgentSession,
  | "invokePluginWireService"
  | "invokePortablePresentationAction"
  | "listPluginWireServices"
...
test("RPC projects portable views and brokers versioned extension services and actions", async () => {
  ...
  const sessionFixture = {
    ...
    listPluginWireServices() { return [descriptor]; },
    async invokePluginWireService(
      request: Parameters<BrokerAgentSession["invokePluginWireService"]>[0],
    ) {
      return {
...
test("RPC snapshots presentations created while a real AgentSession binds extensions", async (context) => {
  ...
  const dispatcher = new RpcRuntimeDispatcher({
    runtime,
    output(value) { outputs.push(value); },
    async bindSession(selected) { await selected.bindPlugins({ mode: "rpc" }); },
  });
