...
import { createRpcPluginErrorEvent } from "../../src/interfaces/rpc-error.js";
import type { RpcPluginUiRequest } from "../../src/interfaces/rpc-protocol.js";
import { MAX_RPC_LINE_BYTES } from "../../src/interfaces/rpc.js";

function capture() {
  const requests: RpcPluginUiRequest[] = [];
  const bridge = new RpcPluginUiBridge({ emit(request) { requests.push(request); } });
...
test("RPC extension dialogs cap unanswered requests and recover capacity after response and abort", async () => {
  ...
  const requests: RpcPluginUiRequest[] = [];
  const bridge = new RpcPluginUiBridge({
    emit(request) {
...
test("RPC extension dialog timeouts accept exact bounds and reject invalid values", async () => {
  ...
  for (const timeout of [0, 3_600_001, 1.5, Number.NaN, Number.POSITIVE_INFINITY]) {
    await assert.rejects(
      ...
      /Plugin UI timeout must be from 1 through 3600000 milliseconds/u,
    );
  }
...
test("RPC extension identity is present and UTF-8 bounded on every emitted request", () => {
  ...
  if (extensionId === undefined) assert.fail("Plugin request omitted its owner ID");
...
test("RPC extension errors are owner-identified and every untrusted field is UTF-8 bounded", () => {
  const event = createRpcPluginErrorEvent({
  ...
  assert.equal(createRpcPluginErrorEvent({
