...
import { runPackageCommand, runProjectPackageCommand } from "../../src/cli/plugins-command.js";
...
test("plugins doctor honors offline mode without creating a durable session", async (t) => {
  ...
  const result = spawnSync(process.execPath, [
    ...
    "plugins", "doctor", "--json", "--offline", "--workspace", workspace,
  ], {
    cwd: resolve("."),
    env: environment,
    encoding: "utf8",
...
test("direct package management persists sources and never copies or deletes local packages", async (t) => {
  ...
  assert.deepEqual(installed.plugins, ["../package"]);
...
test("offline legacy updates fail without printing text or JSON success", async (t) => {
  ...
  const cases = [
    ...
    ["plugins", "update", "--all", "--offline", "--json", "--workspace", workspace],
  ];
...
test("project package commands require trust before mutating project settings", async (t) => {
  ...
  assert.deepEqual(configured.plugins, ["../../package"]);
});
