...
import { applyRuntimePluginFlags, resolveRuntimePluginFlags } from "../../src/cli/plugin-flags.js";
...
test("runtime extension flags are parsed after discovery without consuming a boolean flag's prompt", () => {
  ...
  const resolved = resolveRuntimePluginFlags(bootstrap.unknownFlags, definitions);
...
test("runtime flag resolution reports unknown flags and missing string values", () => {
  const unknown = resolveRuntimePluginFlags(parseArgs(["--unknown", "prompt"]).unknownFlags, definitions);
  assert.match(unknown.diagnostics[0]?.message ?? "", /Unknown option/u);
  const missing = resolveRuntimePluginFlags(parseArgs(["--custom-mode"]).unknownFlags, definitions);
...
test("boolean extension flags are enabled by presence while omitted values retain defaults", () => {
  const explicit = resolveRuntimePluginFlags(parseArgs(["--plan=false", "prompt"]).unknownFlags, definitions);
  assert.equal(explicit.values.get("plan"), true);
  const omitted = resolveRuntimePluginFlags(parseArgs(["prompt"]).unknownFlags, definitions);
...
test("resolved CLI values are visible to extension lifecycle handlers", async (t) => {
  ...
  const parsed = applyRuntimePluginFlags(parseArgs(["--plan", "--custom-mode", "strict"]), host);
