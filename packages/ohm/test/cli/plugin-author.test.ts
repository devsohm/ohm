...
function authorCli(...args: string[]) {
  return spawnSync(process.execPath, ["--import", "tsx", resolve("src/bin/ohm.ts"), ...args], {
...
async function promptFixture(t: TestContext): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), "ohm-author-prompts-"));
  t.after(async () => await rm(root, { recursive: true, force: true }));
  await mkdir(join(root, "prompts"));
  await writeFile(join(root, "package.json"), JSON.stringify({
    name: "author-prompts-fixture",
    version: "1.0.0",
    files: ["prompts"],
    ohm: { prompts: ["prompts"] },
  }));
  await writeFile(join(root, "prompts", "review.md"), "Review the selected change.\n");
  return root;
}

test("plugin author tooling validates and inspects the exact npm pack file set", async (t) => {
  const { root } = await fixture(t);
  const validation = await validatePluginPackage(root);
  assert.equal(validation.package.id, "author-tool-fixture");
  assert.equal(validation.package.hostVersionRange, ">=0.2.0 <0.3.0");
  ...
  const inspected = await inspectPluginPackage(root);
  ...
  await assert.rejects(inspectPluginPackage(root), /ENOENT/u);
});

test("plugin author package manifests enforce the exact 1 MiB bound", async (t) => {
  ...
  assert.equal((await validatePluginPackage(roots[0]!)).package.name, "bounded-author-exact");
  await assert.rejects(
    validatePluginPackage(roots[1]!),
    /package\.json must be a regular file no larger than 1048576 bytes/u,
  );
});

test("plugin author validation counts active declared contributions sharing files", async (t) => {
  ...
  assert.deepEqual((await validatePluginPackage(root)).package.contributions, expected);
  assert.deepEqual((await inspectPluginPackage(root)).validation.package.contributions, expected);
});

test("plugin author tooling ignores an arbitrary npm entry point and a broken PATH shim", async (t) => {
  const { root } = await fixture(t);
...
test("plugins authoring dispatches as a CLI subcommand instead of model input", async (t) => {
  const { root } = await fixture(t);
  const result = authorCli("plugins", "validate", root, "--json");
  ...
  const pretty = authorCli("plugins", "smoke", root);
...
test("plugin author smoke and refresh dispose every author generation", async (t) => {
  const { root, log } = await fixture(t);
  assert.deepEqual(await smokePluginPackage(root), {
  ...
  assert.deepEqual(await refreshPluginPackage(root), {
    packageId: "author-tool-fixture",
    runtimeEntries: 1,
    toolCount: 1,
    commandCount: 1,
...
for (const failure of ["disposal", "activation"]) {
  test(`extension author refresh reports candidate ${failure} failure without masking it`, async (t) => {
    const { root, log } = await fixture(t);
    const countFile = join(root, "activation-count");
    await writeFile(countFile, "0");
    await writeFile(join(root, "extensions", "index.mjs"), `
      import { appendFile, readFile, writeFile } from "node:fs/promises";
      export default async function activate(api) {
        const generation = Number(await readFile(${JSON.stringify(countFile)}, "utf8")) + 1;
        await writeFile(${JSON.stringify(countFile)}, String(generation));
        api.onDispose(async () => {
          await appendFile(${JSON.stringify(log)}, "disposed:" + generation + "\\n");
          if (generation === 2 && ${JSON.stringify(failure)} === "disposal") {
            throw new Error("candidate disposal failed");
          }
          if (generation === 1 && ${JSON.stringify(failure)} === "activation") {
            throw new Error("active cleanup failed");
          }
        });
        if (generation === 2 && ${JSON.stringify(failure)} === "activation") {
          throw new Error("candidate activation failed");
        }
      }
    `);

    await assert.rejects(refreshPluginPackage(root), new RegExp(`candidate ${failure} failed`, "u"));
    assert.deepEqual((await readFile(log, "utf8")).trim().split("\n").sort(), ["disposed:1", "disposed:2"]);
  });
}

test("plugin author pack emits one reviewed artifact and report aggregates every check", async (t) => {
  const { root } = await fixture(t);
  const destination = join(root, "artifacts");
  const packed = await packPluginPackage(root, destination);
  ...
  await assert.rejects(packPluginPackage(root, destination), /already exists/u);
  ...
  const report = await reportPluginPackage(root);
...
test("plugin author pack refuses a pre-existing artifact symlink without overwriting its target", async (t) => {
  const { root } = await fixture(t);
  const destination = join(root, "artifacts");
...
test("an authored archive installs, refreshes, removes, and retains workspace config", async (t) => {
  const { root } = await fixture(t);
  const packed = await packPluginPackage(root, join(root, "artifacts"));
  ...
  const activate = async (args: "write-config" | "read-config"): Promise<string | undefined> => {
    ...
    const host = await loadDirectPlugins(selected.map((entry) => entry.path), {
      workspace: root,
      dataRoot,
    ...
    try {
      ...
      const paths = host.pluginDataPaths(selected[0]!.path);
      assert.ok(paths);
...
test("plugin author report retains actionable failures without activating invalid code", async (t) => {
  ...
  const cliReport = authorCli("plugins", "report", root, "--json");
  ...
  const cliValidate = authorCli("plugins", "validate", root, "--json");
  assert.equal(cliValidate.status, 1);
  assert.equal(cliValidate.stdout, "");
  assert.match(cliValidate.stderr, /^ohm:/u);
});

test("plugin author verify exercises the packed install and leaves source package state alone", async (t) => {
  const { root } = await fixture(t);
  const report = await verifyPluginPackage(root);
  assert.equal(report.status, "success", JSON.stringify(report));
  assert.deepEqual(report.checks.map((entry) => entry.name), ["validate", "inspect", "smoke", "refresh", "packed"]);
  assert.equal(report.checks.every((entry) => entry.status === "success"), true);
  await assert.rejects(access(join(root, "node_modules")), /ENOENT/u);
  await assert.rejects(access(join(root, ".ohm")), /ENOENT/u);
  assert.equal((await readdir(root)).some((name) => name.endsWith(".tgz")), false);
});

test("plugin author verify accepts a prompt-only package without a runtime factory", async (t) => {
  const root = await promptFixture(t);

  assert.equal((await validatePluginPackage(root)).package.enabled, true);
  const report = await verifyPluginPackage(root);
  assert.equal(report.status, "success", JSON.stringify(report));
  assert.deepEqual(report.checks.map((entry) => [entry.name, entry.status]), [
    ["validate", "success"],
  ...
  assert.deepEqual(report.checks.find((entry) => entry.name === "smoke")?.detail, {
    packageId: "author-prompts-fixture",
    runtimeEntries: 0,
    toolCount: 0,
    commandCount: 0,
    providerCount: 0,
  ...
  const smoke = authorCli("plugins", "smoke", root, "--json");
  assert.equal(smoke.status, 0, smoke.stderr);
  assert.deepEqual(JSON.parse(smoke.stdout), report.checks.find((entry) => entry.name === "smoke")?.detail);
  assert.deepEqual((await readdir(root)).sort(), ["package.json", "prompts"]);
});

test("plugin author verify loads every declarative resource kind from source and archive", async (t) => {
  const root = await promptFixture(t);
  await mkdir(join(root, "skills", "review"), { recursive: true });
  await mkdir(join(root, "themes"));
  await writeFile(join(root, "skills", "review", "SKILL.md"), "---\nname: review\ndescription: Review a change.\n---\nRead the diff.\n");
  await writeFile(join(root, "themes", "review.json"), JSON.stringify({
    schemaVersion: 1, name: "review", base: "dark", styles: { accent: { foreground: 81 } },
  }));
  await writeFile(join(root, "package.json"), JSON.stringify({
    name: "author-resources-fixture",
    version: "1.0.0",
    files: ["skills", "prompts", "themes"],
    ohm: { skills: ["skills"], prompts: ["prompts"], themes: ["themes"] },
  }));
  await mkdir(join(root, ".ohm", "extensions"), { recursive: true });
  await writeFile(join(root, ".ohm", "extensions", "unrelated.mjs"), "throw new Error('Unrelated project code must not activate');\n");
  await mkdir(join(root, ".ohm", "themes"));
  await writeFile(join(root, ".ohm", "themes", "unrelated.json"), "invalid unrelated project theme");

  assert.deepEqual((await validatePluginPackage(root)).package.contributions, {
    skillRoots: 1, prompts: 1, commands: 0, themes: 1, runtime: 0,
  });
  const report = await verifyPluginPackage(root);
  assert.equal(report.status, "success", JSON.stringify(report));
  assert.equal(report.checks.at(-1)?.name, "packed");
  assert.deepEqual((await readdir(root)).sort(), [".ohm", "package.json", "prompts", "skills", "themes"]);
});

test("plugin author resource checks reject empty, malformed, and missing contributions", async (t) => {
  const root = await promptFixture(t);
  const prompt = join(root, "prompts", "review.md");
  await writeFile(prompt, "---\ndescription: [\n---\nInvalid metadata.\n");
  for (const check of [smokePluginPackage, refreshPluginPackage]) {
    await assert.rejects(check(root), /Prompt template metadata is invalid YAML/u);
  }

  await writeFile(prompt, "Valid prompt.\n");
  await writeFile(join(root, "package.json"), JSON.stringify({
    name: "author-invalid-resources", version: "1.0.0",
    ohm: { skills: ["skills"], prompts: ["prompts"], themes: ["themes"] },
  }));
  await mkdir(join(root, "themes"));
  await mkdir(join(root, "skills", "broken"), { recursive: true });
  await writeFile(join(root, "themes", "broken.json"), "not JSON");
  await assert.rejects(smokePluginPackage(root), /Resource loading reported:/u);
  await rm(join(root, "themes", "broken.json"));

  await writeFile(join(root, "skills", "broken", "SKILL.md"), "Missing description.\n");
  await assert.rejects(refreshPluginPackage(root), /skill metadata needs a non-blank description/u);
  await rm(join(root, "skills", "broken", "SKILL.md"));

  await rm(prompt);
  await assert.rejects(smokePluginPackage(root), /does not contribute|no enabled contributions/u);
  await writeFile(join(root, "package.json"), JSON.stringify({
    name: "author-missing-resource", version: "1.0.0", ohm: { prompts: ["missing.md"] },
  }));
  await assert.rejects(refreshPluginPackage(root), /missing\.md|does not contribute/u);
});

test("plugin author verify rejects declarative resources omitted from the archive", async (t) => {
  const root = await promptFixture(t);
  await writeFile(join(root, "omitted.md"), "A second declared prompt.\n");
  await writeFile(join(root, "package.json"), JSON.stringify({
    name: "author-omitted-resource", version: "1.0.0", files: ["prompts"], ohm: { prompts: ["prompts", "omitted.md"] },
  }));
  const report = await verifyPluginPackage(root);
  assert.equal(report.status, "error");
  assert.equal(report.checks.slice(0, -1).every((entry) => entry.status === "success"), true);
  assert.equal(report.checks.at(-1)?.name, "packed");
  assert.match(report.checks.at(-1)?.summary ?? "", /omitted\.md/u);
});

test("plugin author verify finds imports omitted from an otherwise valid archive", async (t) => {
  const { root } = await fixture(t);
  await writeFile(join(root, "helper.mjs"), "export const label = 'probe';\n");
  const entry = join(root, "extensions", "index.mjs");
  await writeFile(entry, `import { label } from '../helper.mjs';\n${await readFile(entry, "utf8")}\nvoid label;\n`);
  const report = await verifyPluginPackage(root);
  assert.equal(report.status, "error");
  assert.equal(report.checks.slice(0, -1).every((check) => check.status === "success"), true);
  assert.equal(report.checks.at(-1)?.name, "packed");
  assert.match(report.checks.at(-1)?.summary ?? "", /helper/u);
});

test("plugin author verify honors cancellation before evaluating a package", async (t) => {
  const { root, log } = await fixture(t);
  for (const check of [verifyPluginPackage, smokePluginPackage, refreshPluginPackage]) {
    await assert.rejects(check(root, AbortSignal.abort(new Error("verify cancelled"))), /verify cancelled/u);
  }
  await assert.rejects(access(log), /ENOENT/u);
});
