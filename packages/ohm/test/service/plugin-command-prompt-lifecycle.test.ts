...
for (const surface of surfaces) {
  test(`prompt-dispatched extension commands can use idle session actions in ${surface.name}`, async (context) => {
    ...
    let callbackMode: PluginMode | undefined;
    let waited = false;
    ...
    let actions: PluginCommandContextActions | undefined;
    ...
    const bindings: PluginBindings = { mode: surface.mode };
    if (actions !== undefined) bindings.commandContextActions = actions;
    await session.bindPlugins(bindings);
