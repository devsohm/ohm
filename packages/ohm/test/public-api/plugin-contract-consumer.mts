import {
  ...
  type PluginActions,
  type PluginAPI,
  type PluginCommandContextActions,
  type PluginContext,
  ...
  type PluginJobId,
  type PluginJobListOptions,
  type PluginJobOperation,
  type PluginJobService,
  type PluginJobStartOptions,
  type PluginJobState,
  ...
  type PluginMode,
  type PluginRuntime,
  type PluginSessionDelivery,
  type PluginUIRouteDefinition,
  type PluginUIRouteHost,
  type PluginUIRouteOpenOptions,
  type PluginUIRouteRegistration,
  type PluginUIRouteService,
  type PluginUIRouteSnapshot,
  type PluginUISlotContribution,
  type PluginUISlotPath,
  type PluginUISlotPlacement,
  type PluginUISlotRegistration,
  type PluginUISlotService,
...
export interface ExtensionApiExportFixture {
  ...
  sessionDelivery: PluginSessionDelivery;
  modelCompletion: PluginModelCompletion;
  loadResult: LoadPluginsResult;
  jobs: PluginJobService;
  jobContext: PluginJobContext;
  jobId: PluginJobId;
  jobOperation: PluginJobOperation;
  jobStart: PluginJobStartOptions;
  jobState: PluginJobState;
  jobList: PluginJobListOptions;
  jobWait: PluginJobWaitOptions;
  jobStatus: PluginJobStatus;
}

export interface ExtensionUIRouteExportFixture {
  definition: PluginUIRouteDefinition;
  host: PluginUIRouteHost;
  options: PluginUIRouteOpenOptions;
  registration: PluginUIRouteRegistration;
  service: PluginUIRouteService;
  snapshot: PluginUIRouteSnapshot;
}

export interface ExtensionUISlotExportFixture {
  paths: typeof PLUGIN_UI_SLOT_PATHS;
  path: PluginUISlotPath;
  placement: PluginUISlotPlacement;
  contribution: PluginUISlotContribution;
  registration: PluginUISlotRegistration;
  service: PluginUISlotService;
}
...
export interface ExtensionEventExportFixture {
  ...
  event: PluginEvent;
  eventMap: PluginEventMap;
  eventResultMap: PluginEventResultMap;
...
declare const extensionApi: PluginAPI;
const extensionFacetService: PluginFacetService = extensionApi.facets;
void extensionFacetService;
type MissingExtensionApiMember = Exclude<keyof PluginAPI, (typeof extensionApiMembers)[number]>;
...
const extensionEventNames = [
  ...
  "tool_call",
  "tool_result",
] as const satisfies readonly (keyof PluginEventMap)[];
type MissingExtensionEvent = Exclude<keyof PluginEventMap, (typeof extensionEventNames)[number]>;
...
} satisfies { [K in keyof PluginEventResultMap]: PluginEventResultMap[K] };

declare const api: PluginAPI;
api.on("context", (event) => ({ messages: event.messages.map((message) => {
  if (message.role !== "user" && message.role !== "assistant" && message.role !== "toolResult") return message;
  const contextId: string | undefined = message.contextId;
  if (contextId !== undefined) message.contextId = contextId;
  return message;
}) }));
declare const extensionContext: PluginContext;
declare const resultEvent: ToolResultEvent;

api.sendUserMessage("/review", { expandPromptTemplates: true });
const sessionDelivery: PluginSessionDelivery = extensionContext.sessionDelivery;
void sessionDelivery.sendMessage({
  customType: "public-contract",
  content: "accepted",
  display: false,
});
const completeModel: PluginModelCompletion = extensionContext.modelRegistry.complete;
...
function exerciseUiRouteFacade(context: PluginContext): void {
  const service: PluginUIRouteService = context.ui.routes;
  const definition: PluginUIRouteDefinition = {
    title: "Review details",
    render(host) {
      const selectedHost: PluginUIRouteHost = host;
      return {
        render() {
  ...
  const registration: PluginUIRouteRegistration = service.register("review-details", definition);
  const options: PluginUIRouteOpenOptions = { data: { source: "public-contract" } };
  registration.open(options);
  service.open(registration.name, options);
  const snapshots: readonly PluginUIRouteSnapshot[] = service.list();
  const current: PluginUIRouteSnapshot | undefined = service.current();
  service.close();
...
api.on("project_trust", (event, context) => {
  ...
  const mode: PluginMode = context.mode;
  void type;
  void mode;
...
function registerEvent<K extends keyof PluginEventMap>(name: K, handler: PluginHandler<K>): void {
  api.on(name, handler);
}
...
const invalidMode: PluginMode = "embedding";
