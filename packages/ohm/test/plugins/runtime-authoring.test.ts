...
declare global {
  var __packagedProvenanceApi: PluginAPI;
  var __looseProvenanceApi: PluginAPI;
  ...
  var __commandCatalogApi: PluginAPI;
  var __unifiedCommandApi: PluginAPI;
  ...
  var __capturedCommandContext: PluginCommandContext;
  var __capturedListenerContext: PluginContext;
  var __capturedShortcutContext: RuntimeDirectCommandContext;
  var __generationBoundProviders: GenerationBoundProviderView[];
  var __capturedModelComplete: PluginModelRegistry["complete"];
  ...
  var __authoringFirstApi: PluginAPI;
  ...
  var __authoringLateApi: PluginAPI;
  var __authoringLateShortcut: boolean;
  var __authoringShutdown: boolean;
  var __sameIdFirstApi: PluginAPI;
  var __sameIdSecondApi: PluginAPI;
  var __authoringFirstToolApi: PluginAPI;
  var __authoringSecondToolApi: PluginAPI;
  var __authoringRefreshToolApi: PluginAPI;
  var __authoringLiveToolApi: PluginAPI;
  var __authoringLiveRendererApi: PluginAPI;
  var __authoringReplaceCommandApi: PluginAPI;
  ...
  var __authoringSharedApi: PluginAPI;
...
test("runtime listeners, commands, shortcuts, and tools receive current host mode and trust context", async (context) => {
  ...
  const pluginPaths = host.pluginDataPaths(host.plugins()[0]!.sourcePath);
  assert.ok(pluginPaths !== undefined);
  const expectedPaths = {
    userData: pluginPaths.user,
    workspaceData: pluginPaths.workspace,
  };
...
test("listener provider and auth capabilities expire with their session binding and generation", async (context) => {
  ...
  pluginModelRegistry(modelRegistry).registerProvider(provider);
  calls.length = 0;
...
test("listener model completion follows callback and generation lifetime without following a rebound session", async (context) => {
  ...
  const completeModel: PluginModelRegistry["complete"] = (_model, _context, options) => {
...
test("long conversations cross context and provider hooks without sharing one message value budget", async (context) => {
  const { host } = await fixture(context, [`export default (api) => {
    api.on("context", (event) => ({ messages: JSON.parse(JSON.stringify(event.messages)) }));
    api.on("context", (event) => ({ messages: event.messages.toReversed().slice(0, 4) }));
    api.on("before_provider_request", (event) => event.request
      ? { messages: structuredClone(event.request.messages) }
      : structuredClone(event.payload));
  };\n`]);
  context.after(() => host.close());
  const messages = Array.from({ length: 2_000 }, (_, index) => message(`long-${index}`, "user", "short"));
  const scope = { threadId: "long-thread", runId: "long-run", branch: "main", step: 1 };
  assert.deepEqual(await host.reduceContext({ ...scope, messages }), messages.toReversed().slice(0, 4));
  const request = { messages, tools: [] };
  assert.deepEqual(await host.reduceBeforeProviderRequest({ ...scope, provider: "fixture", model: "fixture", request }), request);
  for (const field of ["messages", "input", "contents"]) {
    const payload = { [field]: messages.map(() => ({ role: "user", content: "short" })), model: "fixture" };
    assert.deepEqual(await host.applyBeforeProviderRequestPayload(payload), payload);
  }
  assert.deepEqual(host.diagnostics(), []);
});

test("conversation collections retain exact aggregate bytes, item counts and absolute depth limits", async (context) => {
  const { host } = await fixture(context, []);
  context.after(() => host.close());
  const maximumBytes = 16 * 1024 * 1024;
  const payload = { input: [{ text: '😀"\\\n' }], tools: [], metadata: { name: "fixture" } };
  payload.input[0]!.text += "x".repeat(maximumBytes - Buffer.byteLength(JSON.stringify(payload)));
  assert.equal(Buffer.byteLength(JSON.stringify(payload)), maximumBytes);
  assert.deepEqual(await host.applyBeforeProviderRequestPayload(payload), payload);
  payload.input[0]!.text += "x";
  await assert.rejects(host.applyBeforeProviderRequestPayload(payload), /UTF-8.*byte/u);

  const emptyPayloads: Array<Record<string, string | null[]>> = [
    { input: [], pad: "" },
    { messages: [], input: [], contents: [], tools: [], pad: "" },
  ];
  for (const emptyPayload of emptyPayloads) {
    emptyPayload.pad = "x".repeat(maximumBytes - Buffer.byteLength(JSON.stringify(emptyPayload)));
    assert.equal(Buffer.byteLength(JSON.stringify(emptyPayload)), maximumBytes);
    assert.deepEqual(await host.applyBeforeProviderRequestPayload(emptyPayload), emptyPayload);
    emptyPayload.pad += "x";
    await assert.rejects(host.applyBeforeProviderRequestPayload(emptyPayload), /UTF-8.*byte/u);
  }

  const messages = Array.from({ length: 100_000 }, () => null);
  assert.deepEqual(await host.applyBeforeProviderRequestPayload({ input: messages }), { input: messages });
  await assert.rejects(host.applyBeforeProviderRequestPayload({ input: messages, messages: [null] }), /at most 0 items/u);
  await assert.rejects(host.applyBeforeProviderRequestPayload({ input: [...messages, null] }), /at most 100000 items/u);
  await assert.rejects(host.applyBeforeProviderRequestPayload({ tools: Array.from({ length: 4_097 }, () => null) }), /at most 4096 items/u);
  await assert.rejects(host.applyBeforeProviderRequestPayload({ input: [Array.from({ length: 8_192 }, () => null)] }), /8192 JSON values/u);
  await assert.rejects(host.applyBeforeProviderRequestPayload({ input: [Array.from({ length: 4_096 }, () => [])] }), /4096 JSON containers/u);

  type Nested = string | { nested: Nested };
  let nested: Nested = "leaf";
  for (let depth = 0; depth < 57; depth += 1) nested = { nested };
  assert.deepEqual(await host.applyBeforeProviderRequestPayload({ input: [nested] }), { input: [nested] });
  await assert.rejects(host.applyBeforeProviderRequestPayload({ input: [{ nested }] }), /levels/u);
});

test("conversation snapshots reject hostile envelopes and collections before invoking accessors", async (context) => {
  const { host } = await fixture(context, []);
  context.after(() => host.close());
  let calls = 0;
  const trap = () => { calls += 1; throw new Error("Hostile property invoked"); };
  const first = message("safe", "user", "safe");
  const accessor = [first];
  Object.defineProperty(accessor, "0", { enumerable: true, get: trap });
  const sparse: CanonicalMessage[] = [];
  sparse.length = 1;
  const extra = [first];
  Object.defineProperty(extra, "hidden", { value: true });
  const inherited = [first];
  Object.setPrototypeOf(inherited, { toJSON: trap });
  const cycle = message("cycle", "user", "safe");
  Reflect.set(cycle, "cycle", cycle);
  const scope = { threadId: "safe-thread", runId: "safe-run", branch: "main", step: 1 };
  for (const messages of [accessor, sparse, extra, inherited, [cycle], new Proxy([first], { get: trap, ownKeys: trap })]) {
    await assert.rejects(host.reduceContext({ ...scope, messages }), /data|dense|vanilla|cycles|proxies/u);
  }
  const event = { ...scope, messages: [first] };
  Object.defineProperty(event, "messages", { enumerable: true, get: trap });
  await assert.rejects(host.reduceContext(event), /data properties/u);
  const tool = { name: "safe", description: "safe", inputSchema: { type: "object" } };
  const tools = [tool];
  Object.defineProperty(tools, "0", { enumerable: true, get: trap });
  await assert.rejects(host.reduceBeforeProviderRequest({ ...scope, provider: "fixture", model: "fixture",
    request: { messages: [first], tools } }), /data entries/u);
  assert.equal(calls, 0);
});

test("context result envelopes reject getters and proxies without reading their data or changing history", async (context) => {
  const { host } = await fixture(context, [`export default (api) => {
    api.on("context", () => ({ get messages() { throw new Error("Getter invoked"); } }));
    api.on("context", () => new Proxy({}, { get(_target, key) {
      if (key === "then") return undefined;
      throw new Error("Proxy data read");
    } }));
  };\n`]);
  context.after(() => host.close());
  const messages = [message("unchanged", "user", "unchanged")];
  assert.deepEqual(await host.reduceContext({ threadId: "safe-thread", runId: "safe-run", branch: "main", messages }), messages);
  assert.equal(host.diagnostics().length, 2);
  assert.ok(host.diagnostics().every((entry) => /data properties|non-proxy/u.test(entry.message)));
});
...
test("duplicate tool ownership distinguishes matching extension IDs at different source paths", async (context) => {
  ...
  try {
    ...
    assert.notDeepEqual(host.pluginDataPaths(firstPath), host.pluginDataPaths(secondPath));
