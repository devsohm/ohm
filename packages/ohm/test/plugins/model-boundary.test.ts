...
test("extension provider models preserve every public wire-compatibility control at the core boundary", () => {
  ...
  assert.deepEqual(pluginModel(internal.find("compatibility-provider", "chat-model")!).compat, {
    supportsStore: true,
    supportsDeveloperRole: true,
    supportsUsageInStreaming: false,
    supportsStrictMode: false,
  ...
  assert.deepEqual(pluginModel(internal.find("compatibility-provider", "responses-model")!).compat, {
    supportsDeveloperRole: true,
    supportsOpenAIGrammarTools: true,
    supportsToolSearch: true,
    supportsExplicitPromptCacheMode: true,
  ...
  assert.deepEqual(pluginModel(internal.find("compatibility-provider", "chat-include-model")!).compat, {
    includeReasoning: true,
  });
  assert.deepEqual(pluginModel(internal.find("compatibility-provider", "messages-model")!).compat, {
    forceAdaptiveThinking: true,
    allowEmptySignature: true,
    supportsEagerToolInputStreaming: false,
    supportsToolReferences: true,
  ...
  assert.deepEqual(pluginModel(internal.find("compatibility-provider", "bedrock-model")!).compat, {
    supportsStrictMode: true,
    supportsPromptCaching: false,
  });
});
...
test("extension model completion uses the active runtime credential and request preparation", async (context) => {
  ...
  const completion = await pluginModelRegistry(runtime.internalRegistry()).complete(
    model,
    requestContext,
    {
...
test("OAuth credentials are normalized at both model-provider boundaries", async () => {
  ...
  const publicOAuth = pluginModelRegistry(new ModelRegistry(models))
...
test("provider-account login preserves its real credential type at both model-provider boundaries", async () => {
  ...
  const publicAccount = pluginModelRegistry(new ModelRegistry(models))
...
test("model directory tool schemas survive the public stream round trip as bounded detached JSON", async () => {
  const models = createModels();
  const internalModel = { ...publicModel("openai-completions"), api: "openai-chat-completions" as const };
  let observedRequest: ProviderRequest | undefined;
  models.setProvider(createProvider({
    id: internalModel.provider,
    auth: { apiKey: { name: "Test key", resolve: async () => ({ auth: { apiKey: "test-key" } }) } },
    models: [internalModel],
    api: { async *stream(request) {
      observedRequest = request;
      yield { type: "response_start", model: request.model } as const;
      yield { type: "response_end", reason: "stop", content: [],
        state: { kind: "chat_completions", assistantMessage: { role: "assistant", content: "" } } } as const;
    } },
  }));
  const provider = pluginModelRegistry(new ModelRegistry(models)).getProvider(internalModel.provider)!;
  const model = provider.getModels()[0]!;
  const schemas: ProviderRequest["tools"][number]["inputSchema"][] = [
    { type: "object", properties: {}, additionalProperties: false },
    { type: "object", properties: { value: { type: "string" } }, required: ["value"], additionalProperties: false },
  ];
  for (const schema of schemas) {
    const request = adapterRequest(model);
    request.tools = [{ name: "live_runtime_marker", description: "Read a marker", inputSchema: schema }];
    const events = await collectEvents(streamFunctionAdapterEvents(model, request, new AbortController().signal,
      (selected, context, options) => provider.streamSimple(selected, context, options)));
    assert.equal(events.at(-1)?.type, "response_end", JSON.stringify(events));
    const received = observedRequest?.tools[0]?.inputSchema;
    assert.deepEqual(structuredClone(received), schema);
    assert.notEqual(received, schema);
    assert.notEqual(received?.properties, schema.properties);
    assert.deepEqual(Reflect.ownKeys(received!), Object.keys(schema));
  }
  let getterCalls = 0;
  const accessor = Object.defineProperty({ type: "object", properties: {} }, "~unsafe", {
    get() { getterCalls += 1; return null; },
  });
  assert.throws(() => provider.streamSimple(model, {
    messages: [], tools: [{ name: "accessor", description: "Must reject", parameters: accessor }],
  }), /only enumerable data properties/u);
  assert.equal(getterCalls, 0);
});
