...
import type { PluginAPI, PluginRegistrationHandle } from "../../src/plugins/direct.js";
...
test("failed service owners publish nothing", async (context) => {
  ...
  let rolledBackHandle: PluginRegistrationHandle | undefined;
  const host = await loadDirectPlugins([], {
    workspace: root,
    inlinePlugins: [
      {
...
test("service names have one owner and exact handles control live registration", async (context) => {
  ...
  let ownerHandle: PluginRegistrationHandle | undefined;
  let owner: PluginAPI | undefined;
  let consumer: PluginAPI | undefined;
