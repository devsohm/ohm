...
import {
  pluginGalleryInstallSource,
  parsePluginGalleryIndex,
  type PluginGalleryPackage,
} from "../../src/plugins/gallery.js";

type GalleryEntryOverrides = Omit<Partial<PluginGalleryPackage>, "securityWarnings"> & {
...
test("gallery index parsing is strict, immutable-source-only, and deterministic", () => {
  const parsed = parsePluginGalleryIndex({
    schemaVersion: 1,
    packages: [
      entry({
        id: "zeta-tools",
  ...
  assert.equal(pluginGalleryInstallSource(parsed.packages[0]!), "npm:@example/review-tools@1.2.3");
  assert.equal(pluginGalleryInstallSource(parsed.packages[1]!), `git:https://example.com/zeta-tools.git#${"a".repeat(40)}`);
});

test("gallery index rejects moving, credential-bearing, duplicate, and incomplete records", () => {
  assert.throws(() => parsePluginGalleryIndex({ schemaVersion: 1, packages: [entry({ version: "latest" })] }), /exact semantic version/u);
  assert.throws(() => parsePluginGalleryIndex({
    schemaVersion: 1,
  ...
  assert.throws(() => parsePluginGalleryIndex({ schemaVersion: 1, packages: [entry(), entry()] }), /duplicate package ID/u);
  assert.throws(() => parsePluginGalleryIndex({ schemaVersion: 1, packages: [entry({ securityWarnings: undefined })] }), /must be an array/u);
  assert.throws(() => parsePluginGalleryIndex({ schemaVersion: 1, packages: [entry({ unexpected: true })] }), /unknown keys/u);
});
...
test("the bundled discovery index is valid and intentionally contains no endorsed packages", async () => {
  ...
  assert.deepEqual(parsePluginGalleryIndex(value), { schemaVersion: 1, packages: [] });
...
test("gallery structural preflight and semantic parsing remain an explicit layered contract", async () => {
  ...
  for (const item of semanticOnlyCases) {
    ...
    assert.throws(() => parsePluginGalleryIndex(item.value), item.error);
  }
});
