...
const PACKAGE_JSON_VALUE = Type.Object({
  ohm: Type.Optional(Type.Object({ entrypoints: Type.Optional(Type.Array(Type.String())) })),
});
...
function directUiContext(overrides: Partial<RuntimeDirectUiContext> = {}): RuntimeDirectUiContext {
  return Object.assign<RuntimeDirectUiContext, Partial<RuntimeDirectUiContext>>({
    capabilities: FULL_TUI_PLUGIN_UI_CAPABILITIES,
...
async function loadExample(
  context: TestContext,
  name: ExampleName,
): Promise<{ host: RuntimePluginHost; workspace: string; session: SessionManager; calls: ActionCall[] }> {
  ...
  const resources = await manager.resolvePluginSources([resolve("examples", name)], { temporary: true });
  ...
  const host = await loadDirectPlugins(resources.extensions.map((entry) => entry.path), {
    workspace,
    mode: "tui",
  ...
  host.setDirectContextHandler(() => ({
    sessionManager: pluginSessionManager(session),
  ...
  host.setDirectActionsHandler({
    ...
    async exec(...values) {
      ...
      return { stdout: name === "code-review" ? "" : "worker output", stderr: "", code: 0, killed: false };
    },
    getActiveTools: () => ["read"],
    getAllTools: () => [{
      name: "read",
...
test("the example corpus includes runtime and declarative package.json packages", async () => {
  ...
  assert.deepEqual(discovered.sort(), [...exampleNames, "task-prompts"].sort());
  for (const name of exampleNames) {
    ...
    const entries = packageJson.ohm?.entrypoints;
    assert.equal(entries?.length, 1);
		assert.equal(entries[0], name === "starter" ? "src/index.ts" : "src/index.mjs");
    await readFile(join(packageRoot, entries[0]!));
  }
});

test("task prompts resolve through the package manager without a runtime factory", async (context) => {
  const root = await mkdtemp(join(tmpdir(), "ohm-example-prompts-"));
  ...
  const resources = await manager.resolvePluginSources([resolve("examples/task-prompts")], { temporary: true });
  ...
  const prompts = loadPromptTemplates({
    ...
    promptPaths: resources.prompts.map((entry) => entry.path),
  });
  assert.deepEqual(prompts.map((prompt) => prompt.name).sort(), ["diagnose-issue", "implement-change", "review-change"]);
});
...
test("tool rendering example traverses the public source TUI barrel and supplies live renderers", async (context) => {
  ...
  const partial = binding.renderCall("read", {
    callId: "partial-example-call",
    name: "read",
    input: {},
    argsComplete: false,
	...
	assert.equal(partial?.lines[0]?.spans[0]?.text.trimEnd(), "Read through plugin · waiting for path");
  const rendered = binding.renderCall("read", {
	...
	assert.equal(rendered?.lines[0]?.spans[0]?.text.trimEnd(), "Read through plugin · README.md");
});
...
test("state and policy example persists bounded workspace memory and tasks and blocks protected paths", async (context) => {
  ...
  const sourcePath = host.plugins()[0]?.sourcePath;
  assert.ok(sourcePath);
  const paths = host.pluginDataPaths(sourcePath);
  assert.ok(paths);
...
test("facet show and update project before publishing and remain retryable after UI failure", async (context) => {
  const { host } = await loadExample(context, "workspace-memory");
  const events: string[] = [];
  const projected: string[] = [];
  host.onPortablePresentation((event) => { events.push(JSON.stringify(event)); });
  let rejectProjection = true;
  host.setDirectUiHandler(() => directUiContext({
    setWidget(_key, content) {
      if (rejectProjection) throw new Error("UI projection failed");
      projected.push(JSON.stringify(content));
    },
  }));
  await host.dispatch("session_start", { reason: "startup", threadId: "projection-retry" });
  await runCommand(host, "memory");
  assert.equal(host.portablePresentations().length, 0);
  assert.equal(events.length, 0);
  rejectProjection = false;
  await runCommand(host, "memory");
  const original = host.portablePresentations();
  assert.equal(original.length, 1);
  assert.equal(events.length, 1);
  const successfulProjections = projected.length;
  rejectProjection = true;
  await runCommand(host, "memory");
  assert.deepEqual(host.portablePresentations(), original);
  assert.equal(events.length, 1);
  assert.equal(projected.length, successfulProjections);
  rejectProjection = false;
  await runCommand(host, "memory");
  assert.notDeepEqual(host.portablePresentations(), original);
...
test("workspace memory loads through the package host and exposes working portable actions", async (context) => {
  const { host } = await loadExample(context, "workspace-memory");
  host.setDirectUiHandler(() => directUiContext());
  await host.dispatch("session_start", { reason: "startup", threadId: "example-memory" });
  await runCommand(host, "memory");
  assert.deepEqual(host.diagnostics(), []);
  const shown = host.portablePresentations()[0];
  assert.ok(shown?.operation === "show");
  const result = await host.invokePortablePresentationAction({
    protocolVersion: 1,
    owner: shown.owner,
    presentationId: shown.presentation.id,
    revision: shown.presentation.revision,
    actionId: "remember",
    input: { text: "Exercise the packed extension too" },
  });
  assert.match(JSON.stringify(result.result), /Exercise the packed extension too/u);
  const updated = host.portablePresentations()[0];
  assert.ok(updated?.operation === "show");
  assert.ok(updated.presentation.revision > shown.presentation.revision);
  await host.dispatch("session_shutdown", { reason: "refresh" });
  assert.equal(host.portablePresentations().length, 0);
  await host.dispatch("session_start", { reason: "startup", threadId: "example-memory-2" });
  await runCommand(host, "memory");
  assert.match(JSON.stringify(host.portablePresentations()), /Exercise the packed extension too/u);
});

test("code review loads as ordinary command and tool registrations", async (context) => {
  const { host, workspace } = await loadExample(context, "code-review");
  assert.equal(host.commands().some((command) => command.name === "review"), true);
  const tool = host.tools().find((entry) => entry.definition.name === "code_review");
  assert.ok(tool);
  ...
  assert.match(result.content, /No tracked changes to review/u);
});
...
test("specialist delegation stays extension-owned and uses bounded managed child processes", async (context) => {
  ...
  await writeFile(fixture, [
    ...
    '  noPluginCode: args.includes("--no-plugins"),',
  ...
  for (const observation of observations) {
    ...
    assert.equal(observation.noPluginCode, true);
