...
test("legacy lock installs remain read-only and migrate only through update all", async (context) => {
  ...
  const legacyHost = getPluginRuntimeHost(loader.getPlugins().runtime);
  assert.deepEqual(legacyHost?.plugins().map((entry) => entry.extensionId), ["declared", "declared"]);
  ...
  await loader.refresh();
  const migratedHost = getPluginRuntimeHost(loader.getPlugins().runtime);
  assert.deepEqual(migratedHost?.commands().map((entry) => entry.name).sort(), ["legacy-live", "legacy-second"]);
  const migratedIds = migratedHost?.plugins().map((entry) => entry.extensionId) ?? [];
...
`);
  ...
  context.after(async () => await getPluginRuntimeHost(loader.getPlugins().runtime)?.close());
  await loader.refresh();
  assert.deepEqual(getPluginRuntimeHost(loader.getPlugins().runtime)?.commands().map((entry) => entry.name), [
    "legacy-unlocked",
  ]);
});
...
test("valid disabled and host-incompatible extension manifests remain intentionally inert", async (context) => {
  ...
  context.after(async () => await getPluginRuntimeHost(loader.getPlugins().runtime)?.close());
  await loader.refresh();
  assert.deepEqual(getPluginRuntimeHost(loader.getPlugins().runtime)?.plugins(), []);
...
test("cancellation in the final post-validation window never commits the staged set", async (context) => {
  ...
  const implementation = (await readFile(new URL("../../src/plugins/project-packages.ts", import.meta.url), "utf8"))
    .replace(/\r\n?/gu, "\n");
...
test("normal refresh activates only locked project resources and a failed update leaves its live generation intact", async (context) => {
  ...
  context.after(async () => await getPluginRuntimeHost(loader.getPlugins().runtime)?.close());
  await loader.refresh();
  const live = getPluginRuntimeHost(loader.getPlugins().runtime);
  assert.ok(live);
  ...
  assert.equal(getPluginRuntimeHost(loader.getPlugins().runtime), live);
  assert.deepEqual(live.commands().map((entry) => entry.name), ["visible"]);
