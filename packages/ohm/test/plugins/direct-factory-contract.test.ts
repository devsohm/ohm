...
import { defineTool, type PluginAPI } from "../../src/plugins/direct.js";
...
declare global {
  ...
  var __ohmDirectApi: PluginAPI | undefined;
  ...
  var __ohmDiscoveryApi: PluginAPI | undefined;
}
...
test("path-first direct loader accepts TypeScript directories and never reuses stale module state", async (context) => {
  ...
  await writeFile(sourcePath, `
    ...
    export default function (ohm: PluginAPI) {
      ohm.registerCommand("first-load", { handler() {} });
    }
  `);
  const first = await loadDirectPlugins([extensionRoot], { workspace: root, activationFailure: "throw" });
  ...
  `);
  const second = await loadDirectPlugins([extensionRoot], { workspace: root, activationFailure: "throw" });
...
test("direct extension source snapshots enforce the inclusive 16 MiB boundary", async (context) => {
  ...
  const exactHost = await loadDirectPlugins([exactPath], {
    workspace: root,
...
test("direct extension activation uses the immutable discovery snapshot", async (context) => {
  ...
  const host = await loadDirectPlugins(paths, { workspace: root, activationFailure: "throw" });
