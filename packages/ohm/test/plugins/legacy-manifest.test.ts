...
import { parseLegacyPluginManifest } from "../../src/plugins/legacy-manifest.js";
...
test("legacy contribution paths reject Windows drive-qualified forms on every host", () => {
  ...
  for (const contributions of cases) {
    assert.throws(() => parseLegacyPluginManifest(manifest(contributions)), /normalized relative path/u);
  }
});
...
test("legacy theme contributions cannot shadow built-in themes", () => {
  for (const name of ["mono", "signal"]) {
    assert.throws(
      () => parseLegacyPluginManifest(manifest({ themes: [{ name, path: "themes/custom.json" }] })),
...
test("legacy integrity paths reject Windows drive-qualified forms on every host", () => {
  assert.throws(() => parseLegacyPluginManifest({
    ...manifest({}),
