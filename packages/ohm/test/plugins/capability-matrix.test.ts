...
import type {
  ...
  PluginContext,
  PluginEventResultMap,
  PluginMode,
  PluginUIContext,
} from "../../src/plugins/direct.js";
import {
  loadDirectPlugins,
  type RuntimePluginChange,
} from "../../src/plugins/runtime.js";

const packageRoot = resolve(dirname(fileURLToPath(import.meta.url)), "../..");
const ALL_HOSTS = ["tui", "print", "json", "rpc", "serve", "sdk"] as const satisfies readonly PluginMode[];
type MissingExtensionMode = Exclude<PluginMode, (typeof ALL_HOSTS)[number]>;
...
const RUNTIME_CHANGE_CATEGORIES = [
  ...
  "session_renderer",
  "tool_renderer",
] as const satisfies readonly RuntimePluginChange[];
type MissingRuntimeChangeCategory = Exclude<RuntimePluginChange, (typeof RUNTIME_CHANGE_CATEGORIES)[number]>;
...
} as const satisfies Record<keyof PluginEventResultMap, string>;
...
const PACKAGE_MANIFEST_VALUE = Type.Object({
  ohm: Type.Optional(Type.Object({
    entrypoints: Type.Optional(Type.Array(Type.String())),
...
type UndocumentedExtensionApiMember = Exclude<keyof PluginAPI, (typeof DOCUMENTED_API_MEMBERS)[number]>;
...
type CapabilityApiMember =
  | keyof PluginAPI
  | keyof PluginContext
  | keyof PluginCommandContext
  | keyof PluginUIContext;
...
const TEST_ONLY_API_EVIDENCE = new Map<string, ReadonlyMap<string, string>>([
  ["bounded-process-execution", new Map([
    ["exec", "test/plugins/direct-factory-contract.test.ts"],
  ])],
  ["durable-jobs", new Map([
    ["jobs", "test/plugins/durable-jobs.test.ts"],
  ])],
  ["optional-facets-portable-presentations-and-wire-services", new Map([
    ["facets", "test/plugins/facets.test.ts"],
  ])],
  ["callback-context-contract", new Map([
    ["abort", "test/plugins/runtime-authoring.test.ts"],
    ["cwd", "test/plugins/runtime-authoring.test.ts"],
    ["getSystemPromptOptions", "test/plugins/runtime-authoring.test.ts"],
    ["hasPendingMessages", "test/service/agent-session.test.ts"],
    ["hasUI", "test/plugins/runtime-authoring.test.ts"],
    ["isIdle", "test/service/agent-session.test.ts"],
    ["isProjectTrusted", "test/plugins/runtime-authoring.test.ts"],
    ["mode", "test/plugins/runtime-authoring.test.ts"],
    ["model", "test/service/agent-session.test.ts"],
    ["modelRegistry", "test/plugins/runtime-authoring.test.ts"],
    ["sessionDelivery", "test/plugins/session-delivery-runtime.test.ts"],
    ["shutdown", "test/service/agent-session.test.ts"],
    ["signal", "test/plugins/runtime-authoring.test.ts"],
    ["thinkingLevel", "test/service/agent-session.test.ts"],
    ["waitForIdle", "test/plugins/runtime-authoring.test.ts"],
  ])],
  ["terminal-workbench-ui", new Map([
    ...
    "setWorkingVisible",
    "theme",
  ].map((member) => [member, "test/plugins/runtime-authoring.test.ts"] as const))],
]);
...
test("the runtime change union matches every emitted category", async () => {
  const source = await readFile(join(packageRoot, "src/plugins/runtime.ts"), "utf8");
...
async function exampleAccessedMembers(relativeRoot: string): Promise<Set<string>> {
  ...
  const resources = manifest.ohm;
  const declaredPaths = [resources?.entrypoints, resources?.skills, resources?.prompts, resources?.themes]
    .flatMap((paths) => paths ?? []);
  assert.equal(declaredPaths.length > 0, true, `${relativeRoot} has no declared resources`);
  for (const path of declaredPaths) await assertPackagePath(`${relativeRoot}/${path}`);
  const members = new Set<string>();
  for (const entry of resources?.entrypoints ?? []) {
...
test("extension capability metadata validates every referenced artifact and example package", async () => {
  const matrix = parseJson(
    ...
    await readFile(join(packageRoot, "docs/plugin-capabilities.json"), "utf8"),
  );
  ...
  for (const capability of matrix.capabilities) {
    ...
    for (const contract of capability.eventContracts ?? []) {
      ...
      assert.equal(
        capability.docs.includes("docs/plugin-events.md"),
        true,
