...
declare global {
  var __configRootMoveApi: PluginAPI | undefined;
}
...
test("project extension configuration survives package and workspace root moves", async (context) => {
  ...
  const firstHost = await loadDirectPlugins([firstPath], {
  ...
  const firstPaths = firstHost.pluginDataPaths(firstPath);
  await firstHost.close();

  const secondHost = await loadDirectPlugins([secondPath], {
  ...
  const secondPaths = secondHost.pluginDataPaths(secondPath);
