...
import type { PluginProcessId } from "../../src/process/managed-process.js";

interface GenerationProbe {
  readonly api: PluginAPI;
  readonly stagedConfig: PluginConfigSnapshot;
  readonly stagedConfigError: string;
  readonly stagedProcessError: string;
  start(script: string, stdin?: "ignore" | "pipe"): PluginProcessId;
}
...
function extensionSource(key: string, generation: number): string {
  return `
    ...
    export default async function activate(ohm: PluginAPI): Promise<void> {
...
async function readPid(api: PluginAPI, id: PluginProcessId): Promise<number> {
...
test("a public-only extension composes durable config with generation-owned processes", { timeout: 15_000 }, async (context) => {
  ...
  const loader = new DefaultResourceLoader({
    ...
    additionalPluginPaths: [sourcePath],
  });
  context.after(async () => {
  ...
  await assert.rejects(
    loader.refresh({ preparePlugins() { throw new Error("candidate rejected"); } }),
