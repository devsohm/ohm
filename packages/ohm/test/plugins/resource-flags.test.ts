...
import { loadPromptTemplates, loadThemes, renderPluginPrompt } from "../../src/plugins/index.js";
import { resourcePathHasMagic } from "../../src/plugins/loose-resources.js";
...
test("resource flags and short aliases parse as repeatable invocation resources", () => {
  ...
  assert.equal(parsed.noPluginCode, true);
...
test("loose prompt templates support frontmatter, arguments, defaults, and slices", async (t) => {
  ...
  assert.equal(
    renderPluginPrompt(prompt!, 'Button "click handler" disabled'),
