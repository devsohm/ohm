...
} from "../../src/plugins/config-store.js";

const SOURCE_PATH = resolve(
  dirname(fileURLToPath(import.meta.url)),
  "../../src/plugins/config-store.ts",
);
...
function store(value: Fixture, writable = (): boolean => true): PluginConfigStore {
  return createPluginConfigStore({
...
test("extension config enforces mandatory compare-and-swap revisions", async (context) => {
  ...
  await assert.rejects(
    ...
    (error) => {
      assert.ok(error instanceof PluginConfigConflictError);
...
test("extension config serializes competing writers across processes", async (context) => {
  ...
  const script = `
    import { createPluginConfigStore, PluginConfigConflictError } from ${JSON.stringify(sourceUrl)};
    ...
    try {
      ...
      if (!(error instanceof PluginConfigConflictError)) throw error;
...
test("extension config cannot replace or remove after its generation becomes stale", async (context) => {
  ...
  const replacement = createPluginConfigStore({
    roots: { user: paths.user, workspace: paths.workspace },
  ...
  let removalChecks = 0;
  const removal = createPluginConfigStore({
    roots: { user: paths.user, workspace: paths.workspace },
