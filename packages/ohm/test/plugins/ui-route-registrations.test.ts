...
import {
  MAX_PLUGIN_UI_ROUTES_PER_GENERATION,
  RuntimeUIRouteOwnerToken,
  RuntimeUIRouteRegistrations,
  UNAVAILABLE_PLUGIN_UI_ROUTES,
  type RuntimeUIRouteOperationSink,
} from "../../src/plugins/runtime-internal/ui-route-registrations.js";
...
test("UI routes enforce names, titles, registration count, and data bounds", () => {
  ...
  for (let index = 0; index < MAX_PLUGIN_UI_ROUTES_PER_GENERATION; index += 1) {
...
test("unavailable and aborted UI route services fail closed and abort cleans mounts", () => {
  ...
  assert.equal(UNAVAILABLE_PLUGIN_UI_ROUTES.current(), undefined);
