...
import { projectLoadedPluginHost } from "../../src/plugins/compat.js";
import {
  pluginModelRegistry,
...
function extension(path: string): Plugin {
  return {
    path,
    resolvedPath: path,
...
function on<K extends keyof PluginEventMap>(
  selected: Plugin,
  event: K,
  ...
  handlers.push(handler as PluginHandler);
...
function command(selected: Plugin, name: string) {
  return {
    name,
...
function actions(): PluginActions {
  return {
    sendMessage() {},
    sendUserMessage() {},
    appendEntry() {},
...
function contextActions(): PluginContextActions {
  return {
    getModel: () => undefined,
    isIdle: () => true,
...
test("zero-argument runtime preserves pre-bind actions, provider queues, and staleness", async () => {
  ...
  assert.equal(getPluginRuntimeHost(runtime), undefined);
...
test("five-argument runner binds actions and keeps projection resolution deterministic", async () => {
  ...
  const command = (owner: Plugin, description: string) => ({
    name: "probe",
    description,
  ...
  const pathsSnapshot = runner.getPluginPaths();
  ...
  flagsSnapshot.clear();
  assert.deepEqual(runner.getPluginPaths(), [first.path, second.path]);
  assert.deepEqual(runner.getActiveTools(), ["read"]);
  assert.equal(runner.getFlags().get("mode")?.default, "first");
...
test("compat callback model completion is bound to callback and runner lifetime", async (context) => {
  ...
  const publicRegistry = pluginModelRegistry(modelRegistry);
  ...
  const completeModel: PluginModelCompletion = (_model, _modelContext, options) => {
  ...
  context.after(async () => await getPluginRuntimeHost(runtime)?.close());
  runner.bindCore(actions(), {
    ...contextActions(),
...
test("standalone callbacks and registrations receive stable owner-specific data paths", async () => {
  ...
  const expected = entries.map((entry) => {
    const paths = host.pluginDataPaths(entry.sourcePath);
    assert.ok(paths);
...
test("standalone runner binds native actions, context, commands, exec, and no-op UI", async () => {
  ...
  const projected = projectLoadedPluginHost(host);
