...
export interface ProjectedPluginError {
...
export function projectPluginError(failure: PluginError): ProjectedPluginError {
  return {
...
export function formatPluginError(failure: PluginError): string {
  ...
  return byteTruncate(escapeTerminal(
    `Plugin error (${extensionId}, ${extensionPath}, ${event}): ${error}`,
  ), 8 * 1_024);
}
