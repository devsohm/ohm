import type { RuntimeShortcutDescription } from "../plugins/runtime.js";
...
export function resolveRuntimeShortcuts(
  ...
  for (const shortcut of shortcuts) {
    ...
    if (reserved.length > 0) {
      diagnostics.push(`Plugin ${shortcut.extensionId} shortcut ${shortcut.shortcut} was skipped because it conflicts with reserved action ${reserved.join(", ")}`);
      continue;
    }
    if (conflicts.length > 0) {
      diagnostics.push(`Plugin ${shortcut.extensionId} shortcut ${shortcut.shortcut} replaces ${conflicts.join(", ")}`);
    }
