...
import { providerInputSchema } from "../tools/parameter-schema.js";
import {
...
export interface PluginProviderConfig {
  ...
  models?: PluginProviderModelConfig[];
  refreshModels?(context: RefreshModelsContext): Promise<PluginProviderModelConfig[]>;
}
...
function internalContextFromPublic(context: Context): ProviderStreamContext {
  ...
  const tools: ProviderToolDefinition[] | undefined = context.tools?.map((tool) => ({
    name: tool.name,
    description: tool.description,
    inputSchema: providerInputSchema(tool.parameters),
    ...optionalProperties(tool.constrainedSampling === undefined ? undefined : { constrainedSampling: structuredClone(tool.constrainedSampling) }),
  }));
  return {
    messages,
...
function internalAuthFromPublic(auth: PluginProvider["auth"]): InternalProviderAuth {
...
function publicAuthFromInternal(auth: InternalProviderAuth): PluginProvider["auth"] {
  ...
  const publicOAuth = internalOAuth?.login === undefined || internalOAuth.refresh === undefined
    ...
    : {
        ...
        async login(interaction: Parameters<NonNullable<PluginProvider["auth"]["oauth"]>["login"]>[0]) {
  ...
  const converted: PluginProvider["auth"] = {};
  const apiKey = auth.apiKey;
  if (apiKey !== undefined) {
    const selected: NonNullable<PluginProvider["auth"]["apiKey"]> = {
      name: apiKey.name,
      resolve: async (input) => await apiKey.resolve(input),
    };
    const login = apiKey.login;
...
function internalProviderConfigModel(
  definition: PluginProviderModelConfig,
...
export type PluginModelCompletion = <TApi extends Api>(
  model: Model<TApi>,
  context: Context,
...
export class PluginModelRegistry {
  ...
  readonly #publicProviders = new Map<string, PluginProvider>();
  readonly #providerViews = new Map<string, PluginProvider>();
  readonly #publicConfigs = new Map<string, PluginProviderConfig>();
  ...
  #publicModel(model: ProviderModel): Model<Api> {
    ...
    const selected = pluginModel(model, preserved?.api);
  ...
  getProvider(provider: string): PluginProvider | undefined {
  ...
  registerProvider(providerName: string, config: PluginProviderConfig): void;
  ...
  getRegisteredProviderConfig(providerName: string): PluginProviderConfig | undefined {
    return this.#publicConfigs.get(providerName);
  }
  getRegisteredNativeProvider(providerName: string): PluginProvider | undefined {
...
const REGISTRY_VIEWS = new WeakMap<InternalModelRegistry, PluginModelRegistry>();

export function pluginModelRegistry(internal: InternalModelRegistry): PluginModelRegistry {
  const existing = REGISTRY_VIEWS.get(internal);
  if (existing !== undefined) return existing;
  const created = new PluginModelRegistry(internal);
...
function internalProviderConfigFromPlugin(
  providerName: string,
  config: PluginProviderConfig,
  registry: PluginModelRegistry,
...
export type PluginThinkingLevel = ThinkingLevel;
