...
export interface ReadonlyPluginSessionManager {
  getCwd(): string;
...
export interface PluginSessionManager extends ReadonlyPluginSessionManager {
...
export function extensionImage(value: ImageBlock): ImageContent {
  if (value.data === undefined) {
    throw new TypeError("Plugin-visible images must contain base64 data; URL-only images cannot cross this boundary");
  }
...
export interface ContextMessageOwner {
  message: CanonicalMessage;
  role: AgentMessage["role"];
}

export interface ContextMessageProjection {
  messages: AgentMessage[];
  owners: ReadonlyMap<string, ContextMessageOwner>;
}

/** Give one context roundtrip explicit owners without persisting or exposing canonical IDs. */
export function extensionContextMessages(messages: readonly CanonicalMessage[]): ContextMessageProjection {
  const owners = new Map<string, ContextMessageOwner>();
  const projected = messages.flatMap((message) => extensionMessages(message).map((value) => {
    const contextId = createId("context");
    owners.set(contextId, { message, role: value.role });
    return { ...value, contextId };
  }));
  return { messages: projected, owners };
}

export function canonicalAgentMessages(
  messages: readonly AgentMessage[],
  owners: ContextMessageProjection["owners"] = new Map(),
): CanonicalMessage[] {
  const usedTokens = new Set<string>();
  const usedIds = new Set<string>();
  return messages.map((message) => {
    const contextId = "contextId" in message ? message.contextId : undefined;
    let previous: CanonicalMessage | undefined;
    if (contextId !== undefined) {
      if (!Value.Check(STRING_VALUE, contextId) || !owners.has(contextId)) {
        throw new TypeError("Context message identity is unknown or no longer active");
      }
      if (usedTokens.has(contextId)) throw new TypeError("Context message identity cannot be used more than once");
      usedTokens.add(contextId);
      const owner = owners.get(contextId)!;
      if (owner.role !== message.role) throw new TypeError("Context replacements cannot change an owned message role");
      previous = owner.message;
    }
    let selected = message;
    if (message.role === "assistant" && previous !== undefined) {
      const providerState = message.providerState === undefined ? extensionProviderState(previous) : message.providerState;
    ...
    const converted = canonicalMessage(selected, previous);
    ...
    // A canonical tool batch projects to several messages; only one can retain its ID.
    if (usedIds.has(converted.id)) converted.id = createId("msg");
    usedIds.add(converted.id);
    return converted;
  });
}
...
class PluginSessionManagerFacade implements PluginSessionManager {
...
const sessionFacades = new WeakMap<SessionManager, PluginSessionManagerFacade>();

function extensionSessionManagerFacade(manager: SessionManager): PluginSessionManagerFacade {
  const existing = sessionFacades.get(manager);
  if (existing !== undefined) return existing;
  const facade = new PluginSessionManagerFacade(manager);
  sessionFacades.set(manager, facade);
  return facade;
}

export function pluginSessionManager(manager: SessionManager): PluginSessionManager {
