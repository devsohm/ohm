...
export interface PluginConfigDataRoots {
...
export interface PluginConfigReadOptions {
  readonly signal?: AbortSignal;
}

export interface PluginConfigWriteOptions extends PluginConfigReadOptions {
...
export class PluginConfigConflictError extends Error {
  ...
  constructor(expectedRevision: string | null, currentRevision: string | null) {
    super("Plugin configuration changed since it was read");
...
const ABSENT_SNAPSHOT: PluginConfigSnapshot = Object.freeze({
  revision: null,
  value: undefined,
});
...
function rootPath(value: string, scope: PluginConfigScope): string {
  if (!Value.Check(STRING_VALUE, value) || value === "" || value.includes("\0") || !isAbsolute(value)) {
    throw new TypeError(`Plugin ${scope} config root must be an absolute path`);
  }
  return resolve(value);
}

function selectedRoot(
  roots: PluginConfigDataRoots,
  scope: PluginConfigScope,
): string {
  if (scope === "user") return roots.user;
  if (scope === "workspace") return roots.workspace;
  throw new TypeError("Plugin config scope must be user or workspace");
}

async function directoryIdentity(root: string): Promise<DirectoryIdentity> {
  const information = await lstat(root);
  if (!information.isDirectory() || information.isSymbolicLink()) {
    throw new Error(`Plugin config root is not a canonical directory: ${root}`);
  }
  const canonical = await realpath(root);
  if (canonical !== root) throw new Error(`Plugin config root is not canonical: ${root}`);
  if (process.platform !== "win32" && (information.mode & 0o077) !== 0) {
    throw new Error(`Plugin config root is not private: ${root}`);
  }
...
async function assertSameDirectory(expected: DirectoryIdentity): Promise<void> {
  ...
  if (current.dev !== expected.dev || current.ino !== expected.ino) {
    throw new Error(`Plugin config root changed during an operation: ${expected.path}`);
  }
}
...
function assertExpectedRevision(value: string | null): void {
  if (value !== null && !/^[a-f0-9]{64}$/u.test(value)) {
    throw new TypeError("Plugin config expectedRevision must be null or a SHA-256 revision");
  }
}
...
function assertJsonValue<T>(root: T): asserts root is T & JsonValue {
  ...
  while (stack.length > 0) {
    ...
    if (Value.Check(STRING_VALUE, value)) {
      ...
      if (minimumBytes > MAX_CONFIG_BYTES) throw new RangeError(`Plugin config exceeds ${MAX_CONFIG_BYTES.toLocaleString()} bytes`);
      continue;
    }
    if (Value.Check(NUMBER_VALUE, value)) {
      if (!Number.isFinite(value)) throw new TypeError("Plugin config must be a JSON value");
      minimumBytes += 1;
      continue;
    }
    if (!isObjectReference(value) || frame.depth >= MAX_JSON_DEPTH) {
      throw new TypeError("Plugin config must be a bounded JSON value");
    }
    if (active.has(value)) throw new TypeError("Plugin config must be an acyclic JSON value");
    active.add(value);
    ...
    if (Array.isArray(value)) {
      ...
      ) throw new TypeError("Plugin config must be a JSON value without sparse or extra array properties");
    ...
    if (prototype !== Object.prototype && prototype !== null) {
      throw new TypeError("Plugin config must be a JSON value with plain objects");
    }
    const keys = Reflect.ownKeys(value);
    for (const key of keys) {
      if (!Value.Check(STRING_VALUE, key)) throw new TypeError("Plugin config must not contain symbol keys");
      const descriptor = Object.getOwnPropertyDescriptor(value, key);
      if (descriptor === undefined || !descriptor.enumerable || !("value" in descriptor)) {
        throw new TypeError("Plugin config must contain only enumerable data properties");
      }
...
function encode(value: JsonValue): EncodedDocument {
  ...
  if (bytes.byteLength > MAX_CONFIG_BYTES) {
    throw new RangeError(`Plugin config exceeds ${MAX_CONFIG_BYTES.toLocaleString()} bytes`);
  }
...
async function readBounded(path: string, root: DirectoryIdentity): Promise<StoredDocument | undefined> {
  ...
  if (initial.isSymbolicLink()) throw new Error(`Plugin config file is a symbolic link: ${path}`);
  if (!initial.isFile()) throw new Error(`Plugin config path is not a regular file: ${path}`);
  ...
  try {
    ...
    ) throw new Error(`Plugin config file changed while opening: ${path}`);
    ...
    ) throw new Error(`Plugin config file changed while reading: ${path}`);
    await assertSameDirectory(root);
    ...
    try {
      ...
      throw new Error(`Plugin config contains invalid JSON: ${path}`, { cause: error });
    }
...
function snapshot(document: StoredDocument | undefined): PluginConfigSnapshot {
...
function assertRevision(
  ...
  if (expectedRevision !== currentRevision) {
    throw new PluginConfigConflictError(expectedRevision, currentRevision);
  }
}
...
export function createPluginConfigStore(options: PluginConfigStoreOptions): PluginConfigStore {
  if (!Value.Check(OBJECT_VALUE, options)) throw new TypeError("Plugin config store options are required");
  if (!Value.Check(FUNCTION_VALUE, options.writable)) throw new TypeError("Plugin config writable check is required");
  const roots = Object.freeze({
    user: rootPath(options.roots?.user, "user"),
    workspace: rootPath(options.roots?.workspace, "workspace"),
  });
  if (roots.user === roots.workspace) throw new Error("Plugin user and workspace config roots must be distinct");

  const pathFor = (scope: PluginConfigScope): ConfigPath => {
  ...
  const requireWritable = (): void => {
    if (!options.writable()) throw new Error("Plugin configuration is not writable by this generation");
  };

  const read = async (
    scope: PluginConfigScope,
    readOptions: PluginConfigReadOptions = {},
  ): Promise<PluginConfigSnapshot> => {
  ...
  const remove = async (
    ...
    return await withFileLock(selected.path, async () => {
      ...
      try {
        ...
        if (errno(error) !== "ENOENT") throw error;
        throw new PluginConfigConflictError(writeOptions.expectedRevision, null);
      }
      await syncDirectory(selected.root);
