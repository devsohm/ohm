...
import type { Api, Model, Provider as PluginProvider } from "@ohm/models";
import type {
...
import { createPluginConfigStore } from "./config-store.js";
...
import type { PluginRuntimeEntry, PluginScope } from "./types.js";
import type {
  PluginUISlotContribution,
  PluginUISlotPath,
  PluginUISlotService,
} from "./capabilities/ui-slots.js";
import type { PluginUIRouteService } from "./capabilities/ui-routes.js";
import { RuntimeUISlotRegistrations } from "./runtime-internal/ui-slot-registrations.js";
import { UNAVAILABLE_PLUGIN_UI_ROUTES } from "./runtime-internal/ui-route-registrations.js";
import { PluginFacetCoordinator } from "./runtime-internal/facet-coordinator.js";
import type {
...
import { PluginUISlotCompositor } from "../tui/ui-slot-compositor.js";
import {
  pluginModelRegistry,
  type PluginModelCompletion,
...
import type {
  ...
  Plugin as DirectPlugin,
  PluginAPI,
  PluginEventMap as DirectPluginEventMap,
  PluginHandler as DirectPluginHandler,
...
import type { PluginSessionDelivery } from "./capabilities/host.js";
...
const MAX_RUNTIME_PLUGIN_JSON_VALUES = 8_192;
const MAX_RUNTIME_PLUGIN_JSON_CONTAINERS = 4_096;
const MAX_RUNTIME_PLUGIN_JSON_DEPTH = 59;
const MAX_RUNTIME_CONVERSATION_BYTES = 16 * 1024 * 1024;
const MAX_RUNTIME_CONVERSATION_MESSAGES = 100_000;
...
const RUNTIME_WIRE_SERVICE_ENDPOINT_VALUE = Type.Unsafe<PluginWireServiceEndpoint>(Type.Object({
  protocolVersion: Type.Literal(PLUGIN_WIRE_SERVICE_PROTOCOL_VERSION),
  name: STRING_VALUE,
  ...
  maxRequestBytes: Type.Integer({
    minimum: 1,
    maximum: PLUGIN_WIRE_SERVICE_LIMITS.maxPayloadBytes,
  }),
...
const DIRECT_PLUGIN_FACTORY_VALUE = Type.Function([Type.Unknown()], Type.Unknown());
...
const PLUGIN_TEXT_CONTENT_VALUE = Type.Object(
  {
...
const PLUGIN_IMAGE_CONTENT_VALUE = Type.Object(
  {
...
const PLUGIN_INPUT_CONTENT_VALUE = Type.Union([
  STRING_VALUE,
  Type.Array(Type.Union([PLUGIN_TEXT_CONTENT_VALUE, PLUGIN_IMAGE_CONTENT_VALUE])),
]);
const PLUGIN_IMAGE_CONTENT_ARRAY_VALUE = Type.Array(PLUGIN_IMAGE_CONTENT_VALUE);
const PLUGIN_USAGE_VALUE = Type.Object(
  {
...
export const DEFAULT_RUNTIME_PLUGIN_ACTIVATION_TIMEOUT_MS = 30_000;
export const DEFAULT_RUNTIME_PLUGIN_LOAD_TIMEOUT_MS = 30_000;
export const DEFAULT_RUNTIME_PLUGIN_SHUTDOWN_TIMEOUT_MS = 5_000;
export const DEFAULT_RUNTIME_RESOURCE_DISCOVERY_TIMEOUT_MS = 30_000;

export type RuntimePluginEvent =
  | "resources_discover"
  | "project_trust"
  | "session_start"
...
export type RuntimeDirectPluginEvent = Exclude<
  RuntimePluginEvent,
  "session_end" | "before_user_shell" | "user_shell" | "theme_change" | "event"
>;
const RUNTIME_DIRECT_PLUGIN_EVENTS: ReadonlySet<RuntimeDirectPluginEvent> = new Set([
...
export type RuntimePluginChange =
  | "tool"
  | "command"
  | "shortcut"
  | "flag"
...
export type RuntimePluginMode = "tui" | "rpc" | "json" | "print" | "serve" | "sdk";
...
export interface RuntimePluginEventMap {
...
export interface RuntimePluginEventResultMap {
...
export interface PluginUICapabilities {
...
export const FULL_TUI_PLUGIN_UI_CAPABILITIES: Readonly<PluginUICapabilities> = Object.freeze({
  dialogs: true,
  notifications: true,
  status: true,
  workingState: true,
...
export const LINE_TUI_PLUGIN_UI_CAPABILITIES: Readonly<PluginUICapabilities> = Object.freeze({
  dialogs: true,
  notifications: true,
  status: false,
  workingState: false,
...
export const RPC_PLUGIN_UI_CAPABILITIES: Readonly<PluginUICapabilities> = Object.freeze({
  dialogs: true,
  notifications: true,
  status: true,
  workingState: false,
...
export const HEADLESS_PLUGIN_UI_CAPABILITIES: Readonly<PluginUICapabilities> = Object.freeze({
  dialogs: false,
  notifications: false,
  status: false,
  workingState: false,
...
export interface RuntimeDirectUiContext
  ...
  readonly capabilities?: Readonly<PluginUICapabilities>;
  readonly slots: PluginUISlotService;
  readonly routes: PluginUIRouteService;
}
...
export type RuntimePluginModelRegistry = PluginModelRegistry;

export interface RuntimeDirectContextSnapshot {
  sessionManager: ReadonlyPluginSessionManager;
...
export type RuntimeDirectContextHandler = (
  target: RuntimePluginSessionTarget | undefined,
  signal: AbortSignal,
...
export type RuntimeDirectProviderConfig = PluginProviderConfig;
...
interface RuntimeDirectMessagingActions {
  ...
  appendEntry<T = unknown>(customType: string, data?: T, provenance?: PluginSessionProvenance): void;
}
...
export type RuntimePluginListenerContextFor<K extends RuntimePluginEvent> =
  K extends "project_trust" ? RuntimeProjectTrustListenerContext : RuntimePluginListenerContext;

export type RuntimePluginListenerEvent<K extends RuntimePluginEvent> =
  K extends RuntimeDirectPluginEvent
    ? DirectPluginEventMap[K]
    : K extends "event" ? RuntimeObservedEvent : RuntimePluginEventMap[K] & { readonly type: K };

export type RuntimePluginListener<K extends RuntimePluginEvent> = (
  value: RuntimePluginListenerEvent<K>,
  context: RuntimePluginListenerContextFor<K>,
) => RuntimePluginEventResultMap[K] | Promise<RuntimePluginEventResultMap[K]>;
...
export interface RuntimeToolContext extends ToolExecutionContext, Omit<RuntimePluginListenerContext, "signal"> {
  readonly extensionId: string;
  readonly sourcePath: string;
  readonly hasUI: boolean;
  readonly mode: RuntimePluginMode;
...
export interface RuntimePluginSessionTarget {
  threadId: string;
  branch?: string;
  signal?: AbortSignal;
}
...
export type RuntimeCatalogOwner =
  | { kind: "builtin" }
  | { kind: "extension"; extensionId: string; sourcePath: string; scope?: PluginScope }
  | { kind: "host" };
...
export interface RuntimeDirectCommandContext extends RuntimePluginListenerContext {
  getSystemPromptOptions(): BuildSystemPromptOptions;
...
export interface RuntimeDirectNewSessionOptions {
  ...
  setup?(sessionManager: PluginSessionManager): Promise<void>;
...
interface RuntimePluginGeneration {
  active: boolean;
  committed: boolean;
  abortController: AbortController;
  entry: PluginRuntimeEntry;
  dataPaths: RuntimePluginDataPaths;
  compatibilityProjection: DirectPlugin;
...
interface StagedActivation {
  ...
  listeners: Array<{ event: RuntimePluginEvent; listener: RuntimePluginListener<RuntimePluginEvent> }>;
...
type DirectCompatibilityHandler = DirectPlugin["handlers"] extends Map<
  string,
  Array<infer Handler>
> ? Handler : never;
type DirectCompatibilityToolRegistration = DirectPlugin["tools"] extends Map<
  string,
  infer Registration
> ? Registration : never;
...
function runtimeDirectListener<K extends RuntimeDirectPluginEvent>(
  listener: DirectPluginHandler<K>,
): RuntimePluginListener<RuntimePluginEvent> {
  ...
  return listener as RuntimePluginListener<RuntimePluginEvent>;
}

function compatibilityHandler<K extends RuntimeDirectPluginEvent>(
  listener: DirectPluginHandler<K>,
...
const UNBOUND_SESSION_MANAGER = Object.freeze<ReadonlyPluginSessionManager>({
  getCwd: () => "",
  getSessionDir: () => "",
...
interface OwnedListener {
  entry: PluginRuntimeEntry;
  generation: RuntimePluginGeneration;
  event: RuntimePluginEvent;
  listener: RuntimePluginListener<RuntimePluginEvent>;
}

function listenerFor<K extends RuntimePluginEvent>(
  owned: OwnedListener,
  event: K,
): RuntimePluginListener<K> {
  ...
  return async (value, context) => {
    ...
    return result as RuntimePluginEventResultMap[K];
  };
}

function invokeRuntimeListener<K extends RuntimePluginEvent>(
  listener: RuntimePluginListener<K>,
  event: RuntimeBoundaryValue,
  context: RuntimePluginListenerContext | RuntimeProjectTrustListenerContext,
): RuntimePluginEventResultMap[K] | Promise<RuntimePluginEventResultMap[K]> {
  // SAFETY: event projection and context selection are both keyed by K immediately before invocation.
  return listener(
    event as RuntimePluginListenerEvent<K>,
    context as RuntimePluginListenerContextFor<K>,
  );
}
...
function runtimeWireServiceEndpoint<Service extends object>(
  value: Service,
): PluginWireServiceEndpoint | undefined {
...
function sharedEventPayload<Input>(value: Input) {
  const snapshot = boundedJsonSnapshot(value, {
    ...
    maximumValues: MAX_RUNTIME_PLUGIN_JSON_VALUES,
    maximumContainers: MAX_RUNTIME_PLUGIN_JSON_CONTAINERS,
    maximumDepth: MAX_RUNTIME_PLUGIN_JSON_DEPTH,
  });
...
function ownerKey(entry: PluginRuntimeEntry): string {
...
function runtimeUiWithRegistries(
  ...
  slots: PluginUISlotService,
  slotsAvailable: boolean,
  routes: PluginUIRouteService,
...
const RUNTIME_HOST_IMPORTS = new Map<string, string>([
  ...
  ["ohm/plugins", new URL(`./index${runtimeHostModuleExtension}`, import.meta.url).href],
...
let runtimeHostVirtualModules: Promise<NonNullable<JitiOptions["virtualModules"]>> | undefined;
...
function deactivateRuntimeRegistrationHandles(generation: RuntimePluginGeneration): void {
...
function conversationArraySnapshot<Input>(
  value: Input,
  label: string,
  maximumItems: number,
  maximumBytes: number,
  ...
  if (isProxy(value) || !Array.isArray(value) || Object.getPrototypeOf(value) !== Array.prototype) {
    throw new TypeError(`${label} must be a non-proxy vanilla array`);
  }
  const length = value.length;
  if (!Number.isSafeInteger(length) || length < 0 || length > maximumItems) {
    throw new TypeError(`${label} must contain at most ${maximumItems} items`);
  }
  const keys = Reflect.ownKeys(value);
  if (keys.length !== length + 1) throw new TypeError(`${label} must be a dense array without extra properties`);
  let bytes = 2 + Math.max(0, length - 1);
  if (bytes > maximumBytes) throw new TypeError(`${label} exceeds its aggregate UTF-8 byte limit`);
  const items: string[] = [];
  for (const key of keys) {
    if (key === "length") continue;
    const index = Value.Check(STRING_VALUE, key) ? Number(key) : -1;
    const descriptor = Reflect.getOwnPropertyDescriptor(value, key);
    if (!Number.isSafeInteger(index) || index < 0 || index >= length || String(index) !== key
      || descriptor?.enumerable !== true || !("value" in descriptor)) {
      throw new TypeError(`${label} must contain only dense enumerable data entries`);
    }
    const item = boundedJsonSnapshot(descriptor.value, {
      label: `${label}[${index}]`,
      maximumBytes: maximumBytes - bytes,
      maximumValues: MAX_RUNTIME_PLUGIN_JSON_VALUES,
      maximumContainers: MAX_RUNTIME_PLUGIN_JSON_CONTAINERS,
      maximumDepth: MAX_RUNTIME_PLUGIN_JSON_DEPTH - itemDepth,
    });
    bytes += item.bytes;
    items[index] = item.serialized;
  }
  return { serialized: `[${items.join(",")}]`, bytes, length };
}

/** Conversation items have separate structural budgets, but share one serialized byte ceiling. */
function cloneConversation<T>(value: T, label: string, maximumItems = MAX_RUNTIME_CONVERSATION_MESSAGES): T {
  if (isProxy(value)) throw new TypeError(`${label} must not contain proxies`);
  if (Array.isArray(value)) {
    const array = conversationArraySnapshot(value, label, maximumItems, MAX_RUNTIME_CONVERSATION_BYTES, 1);
    // SAFETY: each item and the array's descriptors passed bounded JSON validation.
    return JSON.parse(array.serialized) as T;
  }
  if (!isObjectValue(value)) return cloneBounded(value, label);
  const prototype = Object.getPrototypeOf(value);
  if (prototype !== Object.prototype && prototype !== null) throw new TypeError(`${label} must contain only plain objects`);
  const envelope: RuntimeBoundaryRecord = Object.create(null);
  const collections = new Map<string, RuntimeBoundaryValue>();
  for (const key of Reflect.ownKeys(value)) {
    const descriptor = Reflect.getOwnPropertyDescriptor(value, key);
    if (!Value.Check(STRING_VALUE, key) || descriptor?.enumerable !== true || !("value" in descriptor)) {
      throw new TypeError(`${label} must contain only enumerable string data properties`);
    }
    if (["messages", "input", "contents", "tools"].includes(key) && Array.isArray(descriptor.value)) {
      collections.set(key, descriptor.value);
      envelope[key] = 0;
    } else envelope[key] = descriptor.value;
  }
  const shell = boundedJsonSnapshot(envelope, {
    label,
    maximumBytes: MAX_RUNTIME_CONVERSATION_BYTES,
    maximumValues: MAX_RUNTIME_PLUGIN_JSON_VALUES,
    maximumContainers: MAX_RUNTIME_PLUGIN_JSON_CONTAINERS,
    maximumDepth: MAX_RUNTIME_PLUGIN_JSON_DEPTH,
  });
  const snapshot: RuntimeBoundaryRecord = JSON.parse(shell.serialized);
  let bytes = shell.bytes - collections.size; // Replace each serialized 0 with its array.
  let remainingMessages = maximumItems;
  for (const [key, collection] of collections) {
    const array = conversationArraySnapshot(collection, `${label}.${key}`, key === "tools" ? 4_096 : remainingMessages,
      MAX_RUNTIME_CONVERSATION_BYTES - bytes, 2);
    bytes += array.bytes;
    if (key !== "tools") remainingMessages -= array.length;
    snapshot[key] = JSON.parse(array.serialized);
  }
  // SAFETY: the envelope and every collection were detached without changing their JSON-domain structure.
...
function runtimePluginInputContent<Input>(
  value: Input,
  label: string,
): Static<typeof PLUGIN_INPUT_CONTENT_VALUE> {
  if (!Value.Check(PLUGIN_INPUT_CONTENT_VALUE, value)) throw new TypeError(`${label} is invalid`);
  return value;
}

function runtimePluginUsage<Input>(value: Input, label: string): DirectUsage {
  if (!Value.Check(PLUGIN_USAGE_VALUE, value)) throw new TypeError(`${label} is invalid`);
  return value;
}
...
function validDirectAgentMessage<Input>(value: Input): boolean {
  ...
  if (value.role === "user") {
    return Value.Check(NUMBER_VALUE, value.timestamp) && Value.Check(PLUGIN_INPUT_CONTENT_VALUE, value.content);
  }
  if (value.role === "assistant") {
    return Value.Check(NUMBER_VALUE, value.timestamp)
      ...
      && Value.Check(STRING_VALUE, value.model) && Value.Check(PLUGIN_USAGE_VALUE, value.usage)
  ...
  if (value.role === "toolResult") {
    return Value.Check(NUMBER_VALUE, value.timestamp) && Value.Check(STRING_VALUE, value.toolCallId)
      && Value.Check(STRING_VALUE, value.toolName) && Value.Check(PLUGIN_INPUT_CONTENT_VALUE, value.content)
  ...
  if (value.role === "custom") {
    return Value.Check(NUMBER_VALUE, value.timestamp) && Value.Check(STRING_VALUE, value.customType)
      && Value.Check(PLUGIN_INPUT_CONTENT_VALUE, value.content);
  }
...
function runtimeObservedEvent<Input>(value: Input): RuntimeObservedEvent {
  ...
  // SAFETY: this helper is called only for the RuntimePluginEventMap.event branch after a bounded clone.
...
function canonicalMessages<Input>(value: Input, label: string): CanonicalMessage[] {
  const messages = cloneConversation(value, label);
  if (!Array.isArray(messages)) throw new Error(`${label} must be a bounded message array`);
...
function runtimeProviderTools<Input>(value: Input, label: string): ProviderToolDefinition[] {
  const selected = cloneConversation(value, label, 4_096);
  if (!Array.isArray(selected)) throw new Error(`${label} must be a bounded tool array`);
...
function directToolDetails<Input>(value: Input): JsonValue | undefined {
  if (value === undefined) return undefined;
  try {
    const details = cloneBounded(value, "Plugin tool details", MAX_TOOL_RESULT_METADATA_BYTES);
...
function directToolResult(result: DirectAgentToolResult): ToolResult {
  ...
  if (Value.Check(STRING_VALUE, content)) throw new TypeError("Plugin tool content must be an array");
...
function directToolInput<TParams extends TSchema>(
  ...
  if (!Value.Check(schema, input)) throw new TypeError("Plugin tool input does not match its parameter schema");
  return input;
}
  ...
  return {
    ...
    ...optionalProperties(prepareArguments === undefined ? undefined : {
      async prepareInput(input) {
        const prepared = cloneBounded(
          await prepareArguments(cloneBounded(input, "Plugin tool input", MAX_TOOL_INPUT_BYTES)),
          "Plugin tool prepared input",
          MAX_TOOL_INPUT_BYTES,
        );
        if (!isJsonValue(prepared)) throw new TypeError("Plugin tool prepared input must be JSON-safe");
        return prepared;
      },
    }),
    ...
    ...optionalProperties(renderCall === undefined ? undefined : {
      renderCall(view: Readonly<RuntimeToolRenderView>, context: RuntimeUiRenderContext, bridge?: RuntimeToolRenderBridge) {
        ...
        const output = renderCall(
          // SAFETY: the renderer contract describes potentially incomplete model arguments, not execution-validated parameters.
          view.input as Parameters<typeof renderCall>[0],
          directRendererTheme(context, bridge),
...
function directSourceInfo(path: string, scope: PluginScope): SourceInfo {
  return {
    path,
    source: path,
...
function emptyPluginRegistrationMap<Key, Value>(): Map<Key, Value> {
...
function activation(
  ...
  const compatibilityProjection: DirectPlugin = {
    path: entry.sourcePath,
    resolvedPath: entry.sourcePath,
    sourceInfo: compatibilitySourceInfo,
    ...optionalProperties(hidden === undefined ? undefined : { hidden }),
    handlers: emptyPluginRegistrationMap(),
    tools: emptyPluginRegistrationMap(),
    messageRenderers: emptyPluginRegistrationMap(),
    entryRenderers: emptyPluginRegistrationMap(),
    commands: emptyPluginRegistrationMap(),
    flags: emptyPluginRegistrationMap(),
    shortcuts: emptyPluginRegistrationMap(),
  };
  const generation: RuntimePluginGeneration = {
    active: true,
    committed: false,
  ...
  const config = createPluginConfigStore({
    roots: dataPaths,
    signal: generation.abortController.signal,
  ...
  const events: PluginAPI["events"] = {
    on(topicValue, handler) {
      ...
      let handle: PluginRegistrationHandle;
  ...
  const services: PluginAPI["services"] = Object.freeze({
  ...
  const registerRuntimeListener = <K extends RuntimeDirectPluginEvent>(
    event: K,
    listener: DirectPluginHandler<K>,
  ): RuntimePluginListener<RuntimePluginEvent> => {
    assertActive();
    if (!RUNTIME_DIRECT_PLUGIN_EVENTS.has(event)) throw new Error(`Unknown runtime event: ${event}`);
  ...
  let directApi!: PluginAPI;
  const unregisterFacetListener = <K extends RuntimeDirectPluginEvent>(
    event: K,
    listener: RuntimePluginListener<RuntimePluginEvent>,
  ): void => {
    if (!staged.committed) {
  ...
  const facetCoordinator = new PluginFacetCoordinator({
    ...
    showPresentation(definition, signal, project) {
      return host.showPortablePresentation(staged.entry, staged.generation, definition, signal, project);
    },
  ...
  directApi = {
    ...
    facets: Object.freeze({
      async register(definition: PluginFacetDefinition) {
    ...
    registerProvider(providerOrName: PluginProvider | string, config?: RuntimeDirectProviderConfig) {
      assertActive();
...
export interface RuntimePluginHostOptions {
...
const agentSessionMessageUpdateDispatchers = new WeakMap<RuntimePluginHost, AgentSessionMessageUpdateDispatcher>();
...
export class RuntimePluginHost {
  ...
  readonly #errorListeners = new Set<(diagnostic: RuntimePluginDiagnostic) => void>();
  #diagnosticsTruncated = false;
  readonly #rendererFailureKeys = new Set<string>();
  readonly #lifecycle = new AbortController();
  readonly #generations: RuntimePluginGeneration[] = [];
  readonly #disabledCommands = new WeakMap<RuntimePluginGeneration, ReadonlySet<string>>();
  ...
  readonly #changeListeners = new Set<(change: RuntimePluginChange) => void>();
  readonly #requesterThread = new AsyncLocalStorage<{ threadId: string }>();
  readonly #callbackPhase = new AsyncLocalStorage<RuntimePluginEvent>();
  ...
  readonly #nativeUiHosts = new Map<RuntimePluginGeneration, NativeUiHost>();
  readonly #unsafeTerminalHosts = new Map<RuntimePluginGeneration, UnsafeTerminalHost>();
  readonly #uiSlotCompositor = new PluginUISlotCompositor();
  readonly #uiSlotRegistrations = new WeakMap<RuntimePluginGeneration, RuntimeUISlotRegistrations>();
  ...
  constructor(
    workspace: string,
    options: RuntimePluginHostOptions = {},
  ) {
  ...
  reorderCommittedPlugins(sourcePaths: readonly string[]): void {
    ...
    const rank = (generation: RuntimePluginGeneration): number => {
    ...
    const compare = (left: RuntimePluginGeneration, right: RuntimePluginGeneration): number =>
    ...
    const compareOwned = (
      left: { generation: RuntimePluginGeneration },
      right: { generation: RuntimePluginGeneration },
    ...
    for (const generation of this.#generations) {
      ...
      for (const { registration, tool } of generation.committedTools) {
        ...
        } else this.#diagnoseCrossPluginToolCollision(generation.entry, registration.name, prior);
      }
    }
  ...
  async resolveProjectTrust(
    ...
    for (const owned of this.#listeners.get("project_trust") ?? []) {
      ...
      try {
        ...
        const listenerEvent: RuntimePluginListenerEvent<"project_trust"> = Object.freeze({
          type: "project_trust",
          cwd: workspace,
        });
  ...
  async discoverResources(
    ...
    for (const owned of this.#listeners.get("resources_discover") ?? []) {
      ...
      try {
        ...
        const listenerEvent: RuntimePluginListenerEvent<"resources_discover"> = Object.freeze({
  ...
  addDiagnostic(entry: RuntimePluginDiagnostic): void {
  ...
  nativeUi(entry: PluginRuntimeEntry, generation: RuntimePluginGeneration): NativeUiHost {
    this.#assertLive(entry, generation);
  ...
  rollbackNativeUi(generation: RuntimePluginGeneration): void {
  ...
  unsafeTerminal(entry: PluginRuntimeEntry, generation: RuntimePluginGeneration): UnsafeTerminalHost {
    this.#assertLive(entry, generation);
  ...
  rollbackUnsafeTerminal(generation: RuntimePluginGeneration): void {
  ...
  directActions(
    ...
    const actions: RuntimeDirectActionsHandler = {
      ...
      registerProvider(
        providerOrName: PluginProvider | string,
        config?: RuntimeDirectProviderConfig,
      ): void {
  ...
  registerLiveTool(
    ...
    const prior = this.#tools.get(registration.name);
    if (prior !== undefined) {
      if (this.#diagnoseCrossPluginToolCollision(entry, registration.name, prior)) return false;
    }
  ...
  pluginWireServices(): readonly PluginWireServiceDescriptor[] {
    ...
    for (const owned of this.#services.values()) {
      ...
      try {
        ...
        if (
          endpoint === undefined
          || pluginWireServiceRegistryName({ name: endpoint.name, version: endpoint.version })
            !== owned.registration.name
        ) continue;
        const descriptor = describePluginWireServiceEndpoint(
          endpoint,
          owned.entry.extensionId,
        );
        if (descriptors.length >= PLUGIN_WIRE_SERVICE_LIMITS.maxCatalogEntries) {
          throw new RangeError(
            `Plugin wire service catalog exceeds ${PLUGIN_WIRE_SERVICE_LIMITS.maxCatalogEntries} entries`,
          );
        }
        catalogBytes += Buffer.byteLength(JSON.stringify(descriptor), "utf8")
          + (descriptors.length === 0 ? 0 : 1);
        if (catalogBytes > PLUGIN_WIRE_SERVICE_LIMITS.maxCatalogBytes) {
          throw new RangeError(
            `Plugin wire service catalog exceeds ${PLUGIN_WIRE_SERVICE_LIMITS.maxCatalogBytes} bytes`,
          );
        }
        descriptors.push(descriptor);
      } catch (error) {
        if (error instanceof RangeError && error.message.startsWith("Plugin wire service catalog exceeds")) {
          throw error;
        }
  ...
  async invokePluginWireService(
    ...
    if (owned === undefined || !owned.generation.active) {
      throw new TypeError("Plugin wire service is unavailable");
    }
    const endpoint = runtimeWireServiceEndpoint(owned.registration.service);
    if (
      endpoint === undefined
    ...
    ) throw new TypeError("Plugin wire service endpoint is incompatible");
    try {
      return validatePluginWireServiceResponse(
      ...
      return Object.freeze({
        ...
        error: Object.freeze({
          ...
          message: "Plugin wire service handler failed",
        }),
      });
    }
  }
  ...
  #withRequesterThread<T, Input>(event: RuntimePluginEvent, value: Input, operation: () => T): T {
  ...
  #changed(change: RuntimePluginChange, entry: PluginRuntimeEntry): void {
  ...
  #directUiContext(
    ...
    if (bound !== undefined) {
      ...
      return this.#directUiWithPromptLifecycle(runtimeUiWithRegistries(
        ...
        slotsAvailable,
        routesAvailable ? bound.routes : UNAVAILABLE_PLUGIN_UI_ROUTES,
        routesAvailable,
      ));
    }
  ...
  async #withDirectUiPrompt<T>(
    ...
    if (Value.Check(STRING_VALUE, title) && title !== "") {
      try { safeTitle = bounded(title, "Plugin UI prompt title", 4 * 1024); }
  ...
  #directModelRegistry(
    ...
    completeModel: PluginModelCompletion | undefined,
    signal: AbortSignal,
    assertCurrent?: () => void,
  ): PluginModelRegistry {
    ...
    const providerViews = new WeakMap<PluginProvider, PluginProvider>();
    const guardedProvider = (provider: PluginProvider): PluginProvider => {
    ...
    const getProvider = (provider: string): PluginProvider | undefined => {
      const selected = registry.getProvider(provider);
      return selected === undefined ? undefined : guardedProvider(selected);
    };
    const getRegisteredNativeProvider = (provider: string): PluginProvider | undefined => {
    ...
    const fallbackComplete: PluginModelCompletion = (model, context, options) =>
      registry.complete(model, context, options);
    const completeRequest = completeModel ?? fallbackComplete;
    const complete: PluginModelCompletion = (model, context, options) => {
  ...
  async dispatch<K extends RuntimePluginEvent>(
    event: K,
  ...
  #recordOwnedFailure(entry: PluginRuntimeEntry, operation: string, cause: unknown): void {
    this.addDiagnostic({
      extensionId: entry.extensionId,
      sourcePath: entry.sourcePath,
  ...
  async #reduce<K extends RuntimePluginEvent, T>(
    event: K,
    initial: T,
    step: (
      current: T,
      listener: (value: RuntimePluginEventMap[K]) => RuntimePluginEventResultMap[K] | Promise<RuntimePluginEventResultMap[K]>,
      entry: PluginRuntimeEntry,
  ...
  async reduceInput(event: RuntimeInputEvent, signal?: AbortSignal): Promise<RuntimeInputResult> {
    ...
    const state = await this.#reduce("input", {
      ...
      if (
        selected.images !== undefined && !Value.Check(PLUGIN_IMAGE_CONTENT_ARRAY_VALUE, selected.images)
  ...
  async reduceBeforeAgentStart(event: RuntimeBeforeAgentStartEvent, signal?: AbortSignal): Promise<RuntimeBeforeAgentStartReduction> {
    ...
    const composition = (systemPrompt: string, extensionId: string): PromptCompositionMetadata | undefined => {
      ...
      return {
        ...
        sources: [{
          ...
          source: `extension:${extensionId}:before_agent_start`,
          bytes: Buffer.byteLength(systemPrompt, "utf8"),
          sha256: sha256(systemPrompt),
        }],
    ...
    const snapshot = (current: RuntimeBeforeAgentStartReduction): RuntimeNativeSystemPromptSnapshot => {
      return Object.freeze({
        ...
        ...optionalProperties(current.promptComposition === undefined ? undefined : { composition: structuredClone(current.promptComposition) }),
      });
    };
    const initialState: RuntimeBeforeAgentStartReduction = {
      messages: [],
      systemPrompt: initial.systemPrompt,
      ...optionalProperties(initial.promptComposition === undefined ? undefined : { promptComposition: structuredClone(initial.promptComposition) }),
    };
    const reduced = await this.#reduce("before_agent_start", initialState, async (current, listener, entry) => {
      ...
      const messages = message === undefined
        ...
        : [...current.messages, {
            ...
            content: canonicalInputContent(runtimePluginInputContent(message.content, "Runtime injected message content")),
      ...
      const promptComposition = systemPrompt === current.systemPrompt
        ? current.promptComposition
        : composition(systemPrompt, entry.extensionId);
      return { value: {
        messages,
        systemPrompt,
        ...optionalProperties(promptComposition === undefined ? undefined : { promptComposition }),
      } };
    }, signal === undefined ? {} : { signal });
    if (initial.threadId !== undefined && initial.branch !== undefined) {
      this.#systemPrompts.set(`${initial.threadId}\0${initial.branch}`, snapshot(reduced));
    }
    return reduced;
  }
  ...
  async reduceBeforeProviderRequest(
    ...
    return await this.#reduce("before_provider_request", initial, async (current, listener) => {
      const result = await listener({
        ...identity,
        request: cloneConversation(current, "Runtime before_provider_request listener request"),
      });
      if (result === undefined) return { value: current };
  ...
  async applyBeforeProviderRequestPayload(
    ...
    for (const owned of this.#listeners.get("before_provider_request") ?? []) {
      ...
      try {
        ...
        const exposed = cloneConversation(current, "Direct provider request payload");
        ...
        const replacement = cloneConversation(selected, "Direct provider request replacement");
  ...
  async reduceContext(event: RuntimeContextEvent, signal?: AbortSignal): Promise<CanonicalMessage[]> {
    const initial = cloneConversation(event, "Runtime context event");
    const identity = {
    ...
    return await this.#reduce("context", canonicalMessages(initial.messages, "Runtime context messages"), async (current, listener) => {
      const projection = extensionContextMessages(current);
      const result = await invokeProjectedListener(listener, {
        ...identity,
        messages: projection.messages,
      });
      if (result === undefined) return { value: current };
      const selected = runtimeSessionRecord(result, ["messages"], "Runtime context result");
      ...
      return { value: canonicalAgentMessages(runtimeAgentMessages(selected.messages, "Runtime context messages"), projection.owners) };
    }, signal === undefined ? {} : { signal });
  }
  ...
  async reduceToolResult(
    ...
    const reduced = await this.#reduce("tool_result", initial, async (current, listener) => {
      ...
      if (selected.content !== undefined) {
        const selectedContent = runtimePluginInputContent(selected.content, "Runtime tool result content");
      ...
      const nextUsage = selected.usage === undefined
        ...
        : canonicalUsage(runtimePluginUsage(selected.usage, "Runtime tool result usage"));
        ...
        ...optionalProperties(selectedSummary === undefined || selectedSummaryText === undefined ? undefined : {
          summary: {
            ...
            ...optionalProperties(selectedSummary.usage === undefined ? undefined : {
              usage: canonicalUsage(runtimePluginUsage(selectedSummary.usage, "Runtime tree summary usage")),
            }),
          },
        }),
        ...
        ...optionalProperties(selectedCompaction === undefined || compactionSummary === undefined
          ...
          compaction: {
            ...
            ...optionalProperties(selectedCompaction.usage === undefined ? undefined : {
              usage: canonicalUsage(runtimePluginUsage(selectedCompaction.usage, "Runtime compaction usage")),
            }),
          },
        }),
      }, "Runtime compaction result");
  ...
  #diagnoseCrossPluginToolCollision(
    entry: PluginRuntimeEntry,
    name: string,
    prior: HarnessTool,
  ): boolean {
  ...
  commit(staged: StagedActivation): void {
    ...
    // Plugin tools use first-owner wins across packages. Re-registering within
    ...
    for (const tool of tools) {
      ...
      if (prior === undefined) {
        ...
        this.#diagnoseCrossPluginToolCollision(staged.entry, tool.name, prior);
      }
    }
...
export interface RuntimePluginLoadOptions {
  workspace: string;
  dataRoot?: string;
  ...
  inlinePlugins?: readonly RuntimeInlinePlugin[];
...
export type RuntimeInlinePlugin =
  | ((ohm: PluginAPI) => void | Promise<void>)
  | {
      name: string;
      factory(ohm: PluginAPI): void | Promise<void>;
      hidden?: boolean;
    };

type RuntimeInlinePluginFactory = Extract<RuntimeInlinePlugin, (ohm: PluginAPI) => void | Promise<void>>;

function isRuntimeInlinePluginFactory(
  value: RuntimeInlinePlugin,
): value is RuntimeInlinePluginFactory {
  return Value.Check(DIRECT_PLUGIN_FACTORY_VALUE, value);
}

interface DirectPluginSourceSnapshot {
  entry: PluginRuntimeEntry;
  bytes: Buffer;
}
...
async function activateRuntimePluginEntries(
  ...
  for (const { entry, bytes } of entries) {
    ...
    try {
      ...
      let dataPaths: RuntimePluginDataPaths;
      try {
...
async function activateInlinePlugins(
  host: RuntimePluginHost,
  inlinePlugins: readonly RuntimeInlinePlugin[],
  options: RuntimePluginLoadOptions,
): Promise<void> {
  validateInlinePlugins(inlinePlugins);
  options.signal?.throwIfAborted();
  const activationTimeoutMs = options.activationTimeoutMs ?? DEFAULT_RUNTIME_PLUGIN_ACTIVATION_TIMEOUT_MS;
  const loadTimeoutMs = options.loadTimeoutMs ?? DEFAULT_RUNTIME_PLUGIN_LOAD_TIMEOUT_MS;
  const shutdownTimeoutMs = options.shutdownTimeoutMs ?? DEFAULT_RUNTIME_PLUGIN_SHUTDOWN_TIMEOUT_MS;
...
async function directPluginEntry(
  pathValue: string,
  index: number,
...
async function resolveDirectPluginEntries(
  ...
  for (const [index, path] of paths.entries()) {
    ...
    try {
      ...
      entries.push(await directPluginEntry(
        path,
        index,
...
export async function appendDirectPlugins(
  host: RuntimePluginHost,
  paths: readonly string[],
  options: RuntimePluginLoadOptions,
): Promise<void> {
...
async function loadResolvedDirectPlugins(
  entries: readonly DirectPluginSourceSnapshot[],
  options: RuntimePluginLoadOptions,
): Promise<RuntimePluginHost> {
  const shutdownTimeoutMs = options.shutdownTimeoutMs ?? DEFAULT_RUNTIME_PLUGIN_SHUTDOWN_TIMEOUT_MS;
  ...
  const host = new RuntimePluginHost(workspace, {
    shutdownTimeoutMs,
    dataRoot,
  ...
  try {
    await activateRuntimePluginEntries(host, entries, { ...options, workspace, dataRoot });
    await activateInlinePlugins(host, options.inlinePlugins ?? [], { ...options, workspace, dataRoot });
  } catch (error) {
    try {
      await host.close();
    } catch (cleanupError) {
