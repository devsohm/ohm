...
import { UNAVAILABLE_PLUGIN_UI_SLOTS } from "./runtime-internal/ui-slot-registrations.js";
...
function extensionFailure<T>(cause: T): Pick<PluginError, "error" | "stack"> {
...
export type PluginErrorListener = (error: PluginError) => void;
...
function contextProxy(context: PluginContext, getSystemPrompt = (): string => context.getSystemPrompt()): PluginContext {
  return {
...
/** Public runner facade over exactly one RuntimePluginHost generation. */
export class PluginRunner {
  readonly #extensions: Plugin[];
  readonly #runtime: PluginRuntime;
  readonly #host: RuntimePluginHost;
  readonly #cwd: string;
  readonly #sessionManager: SessionManager;
  readonly #modelRegistry: ModelRegistry;
  readonly #publicSessionManager: ReturnType<typeof pluginSessionManager>;
  readonly #publicModelRegistry: PluginModelRegistry;
  readonly #lifecycle = new AbortController();
  readonly #errorListeners = new Set<PluginErrorListener>();
  #ui: PluginUIContext = noUi;
  #mode: PluginMode = "print";
  ...
  #getThinkingLevel: PluginActions["getThinkingLevel"] = () => "off";
  #isIdle: () => boolean = () => true;
  ...
  #waitForIdle: PluginCommandContextActions["waitForIdle"] = async () => {};
  #newSession: PluginCommandContextActions["newSession"] = async () => ({ cancelled: false });
  #fork: PluginCommandContextActions["fork"] = async () => ({ cancelled: false });
  #navigateTree: PluginCommandContextActions["navigateTree"] = async () => ({ cancelled: false });
  #switchSession: PluginCommandContextActions["switchSession"] = async () => ({ cancelled: false });
  #refresh: PluginCommandContextActions["refresh"] = async () => {};
  ...
  constructor(
    ...
    this.#publicSessionManager = pluginSessionManager(sessionManager);
    this.#publicModelRegistry = pluginModelRegistry(modelRegistry);
    ...
    this.#host = ensurePluginRuntimeHost(runtime, cwd);
    for (const extension of extensions) {
      const owner = extensionProjectionHost(extension);
      if (owner !== undefined && owner !== this.#host) {
        throw new Error(`Plugin projection belongs to another host generation: ${extension.path}`);
      }
    }
  ...
  bindCore(
    ...
    contextActions: PluginContextActions,
  ...
  #unavailableSessionDelivery(): PluginSessionDelivery {
  ...
  bindCommandContext(actions?: PluginCommandContextActions): void {
    this.#assertActive();
  ...
  setUIContext(uiContext?: PluginUIContext, mode: PluginMode = "print"): void {
    this.#assertActive();
  ...
  getUIContext(): PluginUIContext { return this.#ui; }
  hasUI(): boolean { return this.#ui !== noUi; }
  getPluginPaths(): string[] { return this.#extensions.map((extension) => extension.path); }

  /** @internal Native generation used by first-party mode adapters. */
  getRuntimeHost(): RuntimePluginHost {
    this.#assertActive();
    return this.#host;
  }

  getAllRegisteredTools(): RegisteredTool[] {
    const tools = new Map<string, { extension: Plugin; tool: RegisteredTool }>();
  ...
  getFlags(): Map<string, PluginFlag> {
    const flags = new Map<string, PluginFlag>();
    for (const extension of this.#extensions) {
  ...
  getShortcuts(keybindings: KeybindingsConfig): Map<KeyId, PluginShortcut> {
    ...
    const selected = new Map<KeyId, PluginShortcut>();
    for (const extension of this.#extensions) {
  ...
  invalidate(message = "Plugin runtime context is stale after session replacement or refresh"): void {
  ...
  onError(listener: PluginErrorListener): () => void {
    this.#errorListeners.add(listener);
    return () => this.#errorListeners.delete(listener);
  }

  emitError(error: PluginError): void {
  ...
  #contextModelRegistry(callbackSignal: AbortSignal | undefined): PluginModelRegistry {
    ...
    const registerProvider = (providerOrName: Provider | string, config?: PluginProviderConfig): void => {
      active(() => {
    ...
    const methods = {
      ...
      present: (model: Parameters<PluginModelRegistry["present"]>[0]) => active(() => target.present(model)),
  ...
  createContext(extension?: Plugin): PluginContext {
    const owner = (): PluginRunner => this;
    ...
    return {
      ...
      get sessionDelivery() {
        ...
        return Object.freeze({
          ...
          async sendMessage(...args: Parameters<PluginSessionDelivery["sendMessage"]>) {
            owner().#assertActive();
            await delivery.sendMessage(...args);
          },
          async sendUserMessage(...args: Parameters<PluginSessionDelivery["sendUserMessage"]>) {
            owner().#assertActive();
  ...
  createCommandContext(extension?: Plugin): PluginCommandContext {
    return Object.assign(contextProxy(this.createContext(extension)), {
      ...
      newSession: (...args: Parameters<PluginCommandContextActions["newSession"]>) => {
        this.#assertActive();
        return this.#newSession(...args);
      },
      fork: (...args: Parameters<PluginCommandContextActions["fork"]>) => {
        this.#assertActive();
        return this.#fork(...args);
      },
      navigateTree: (...args: Parameters<PluginCommandContextActions["navigateTree"]>) => {
        this.#assertActive();
        return this.#navigateTree(...args);
      },
      switchSession: (...args: Parameters<PluginCommandContextActions["switchSession"]>) => {
        this.#assertActive();
  ...
  async emit(event: RunnerEmitEvent): Promise<RunnerEmitOutput> {
    ...
    try {
      // SAFETY: RunnerEmitEvent names are a subset of RuntimePluginEvent and dispatchPayload adapts the matching value.
      await this.#host.dispatch(event.type as RuntimePluginEvent, this.#dispatchPayload(event, scope) as never);
    } catch {
  ...
  #usesNativeHost(): boolean {
    if (this.#extensions.length === 0) return this.#host.plugins().length > 0;
  ...
  #handlersFor<K extends keyof PluginEventMap>(eventType: K): Array<{
    extension: Plugin;
    handler: PluginHandler<K>;
  }> {
    ...
    for (const extension of this.#extensions) {
      for (const handler of extension.handlers.get(eventType) ?? []) {
        ...
        handlers.push({ extension, handler: handler as PluginHandler<K> });
      }
    }
    return handlers;
  }
  ...
  #extensionIdForPath(extensionPath: string): string | undefined {
    ...
    return this.#host.plugins().find((entry) => entry.sourcePath === resolvedPath)?.extensionId;
  }
