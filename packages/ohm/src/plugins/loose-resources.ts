...
import type { PluginPromptTemplate, PluginTheme } from "./types.js";
...
): Promise<PluginPromptTemplate[]> {
  const maxFileBytes = trustedResourceFileLimit(options.maxFileBytes);
  const result = new Map<string, PluginPromptTemplate>();
...
export async function loadThemes(
  ...
  const maxFileBytes = trustedResourceFileLimit(options.maxFileBytes);
  const result = new Map<string, PluginTheme>();
