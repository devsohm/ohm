...
import { parseLegacyPluginManifest } from "./legacy-manifest.js";
import {
  loadDirectPlugins,
...
export function projectPackageDeclaredResourceMetadata(
  ...
  const disabledByRoot = new Map(packages.map((entry) => {
    ...
    const declaredSkillRoots = legacyManifest
      ? new Set(parseLegacyPluginManifest(JSON.parse(readTrustedTextFileSync(
        entry.manifestPath,
...
async function readPackageManifest(
  ...
  const legacy = useLegacy ? parseLegacyPluginManifest(legacyJson!.value) : undefined;
  if (legacy !== undefined) {
...
export class ProjectPackageManager {
  ...
  async #stageLock(lock: ProjectPackageLock, signal?: AbortSignal): Promise<void> {
    ...
    try {
      ...
      const resources = await resolver.resolvePluginSources(
      ...
      host = await loadDirectPlugins(paths, {
