...
type PartialToolInput<T> = T extends object
  ? { [Key in keyof T]?: PartialToolInput<T[Key]> }
  : T;
...
> {
  ...
  /** Normalize bounded raw JSON before the coordinator validates the complete parameter schema. */
  ...
  /** Arguments can still be streaming, including incomplete nested objects and array elements. */
  renderCall?(input: PartialToolInput<Static<TParameters>>, theme: Theme, context: ToolRenderContext<TState>): ToolRenderOutput;
  renderResult?(
