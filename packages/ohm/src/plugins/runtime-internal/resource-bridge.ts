...
import type { RuntimePluginDataPaths } from "../runtime.js";
...
): RuntimePluginDataPaths {
  const extensionId = entry.extensionId;
  if (!/^[a-z][a-z0-9._-]{0,62}$/u.test(extensionId)) throw new Error("Plugin ID is invalid");
...
async function securePluginDataDirectory(path: string): Promise<string> {
...
export async function preparePluginDataPaths(
  paths: RuntimePluginDataPaths,
  signal: AbortSignal,
): Promise<RuntimePluginDataPaths> {
  signal.throwIfAborted();
  const user = await securePluginDataDirectory(paths.user);
  signal.throwIfAborted();
  const workspace = await securePluginDataDirectory(paths.workspace);
  signal.throwIfAborted();
