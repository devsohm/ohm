import { RuntimePluginHost } from "../runtime.js";
import type {
  ImageBlock,
  TextBlock,
} from "../../core/types.js";
...
function directInputSchema(parameters: TSchema): Record<string, JsonValue> {
  ...
  if (!isJsonValue(parsed) || !isDirectInputSchema(parsed)) {
    throw new TypeError("Plugin tool inputSchema must be a JSON object");
  }
  return parsed;
}
...
export function ensurePluginRuntimeHost(runtime: PluginRuntime, cwd: string): RuntimePluginHost {
  const existing = getPluginRuntimeHost(runtime);
  if (existing !== undefined) return existing;
  const host = new RuntimePluginHost(cwd);
  attachPluginRuntimeHost(runtime, host);
  return host;
}

export function attachPluginProjection(
  extension: Plugin,
  runtime: PluginRuntime,
): void {
  const host = getPluginRuntimeHost(runtime);
  if (host === undefined) throw new Error("Plugin runtime has no attached host generation");
  const owner = extensionRuntimeOwners.get(extension);
  if (owner !== undefined && owner !== host) throw new Error("Plugin projection belongs to another host generation");
  extensionRuntimeOwners.set(extension, host);
}

export function extensionProjectionHost(extension: Plugin): RuntimePluginHost | undefined {
...
export function createPluginRuntime(): PluginRuntime {
  ...
  const unavailable = (): never => {
    assertActive();
    throw new Error("Plugin runtime actions are unavailable before the session host is bound");
  };
  ...
  runtime = {
    ...
    assertActive,
    invalidate(message) {
      staleMessage ??= message ?? "Plugin runtime context is stale after session replacement or refresh";
    },
...
export function createCompatibilityDirectActions(
  ...
  return {
    sendMessage: active((message, options) => {
      const selected: Parameters<PluginRuntime["sendMessage"]>[0] = {
