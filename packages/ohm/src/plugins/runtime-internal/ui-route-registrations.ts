...
import type {
  PluginUIRouteDefinition,
  PluginUIRouteHost,
  PluginUIRouteOpenOptions,
  PluginUIRouteRegistration,
  PluginUIRouteService,
  PluginUIRouteSnapshot,
} from "../capabilities/ui-routes.js";

export const MAX_PLUGIN_UI_ROUTES_PER_GENERATION = 32;
export const MAX_PLUGIN_UI_ROUTE_DATA_BYTES = 64 * 1024;
...
const unavailable = (): never => {
  throw new Error("Plugin UI routes require the full rich TUI");
};

export const UNAVAILABLE_PLUGIN_UI_ROUTES: PluginUIRouteService = Object.freeze({
...
interface RegistrationState {
  ...
  readonly render: PluginUIRouteDefinition["render"];
...
interface CurrentState {
  ...
  readonly snapshot: PluginUIRouteSnapshot;
}

function routeName(value: string): string {
  if (!Value.Check(ROUTE_NAME_VALUE, value) || !ROUTE_NAME.test(value)) {
    throw new TypeError("Plugin UI route names must match [a-z][a-z0-9-]{0,63}");
  }
  return value;
}

function routeDefinition(value: PluginUIRouteDefinition): Omit<RegistrationState, "name" | "token" | "disposed"> {
  if (!Value.Check(ROUTE_DEFINITION_VALUE, value)) {
    throw new TypeError("Plugin UI route definition must be an object");
  }
  ...
  if (
    ...
    throw new TypeError("Plugin UI route titles must contain 1-256 bytes of single-line terminal-safe text");
  }
...
function routeData(options: PluginUIRouteOpenOptions | undefined): JsonValue | undefined {
  if (options === undefined) return undefined;
  if (!Value.Check(ROUTE_OPEN_OPTIONS_VALUE, options)) {
    throw new TypeError("Plugin UI route open options must be an object");
  }
  if (options.data === undefined) return undefined;
  const snapshot = boundedJsonSnapshot(options.data, {
    label: "Plugin UI route data",
    maximumBytes: MAX_PLUGIN_UI_ROUTE_DATA_BYTES,
  ...
  if (!isJsonValue(parsed)) throw new TypeError("Plugin UI route data did not serialize as JSON");
  return deepFreezeJson(parsed);
}

function listedSnapshot(state: RegistrationState): PluginUIRouteSnapshot {
...
export class RuntimeUIRouteRegistrations {
  ...
  service(available: boolean): PluginUIRouteService {
    const active = (): void => {
      this.#assertActive();
    ...
    const access = (): void => {
      active();
      if (!available) throw new Error("Plugin UI routes require the full rich TUI");
    };
    const open = (
      ...
      access();
      if (this.#opening) throw new Error("Plugin UI route navigation is already in progress");
      if (state.disposed || this.#registrations.get(state.name) !== state) {
        throw new Error(`Plugin UI route registration is no longer active: ${state.name}`);
      }
      ...
      const factory: RuntimeUiComponentFactory<void> = (host) => {
        const routeHostBase = {
          ...
          name: state.name,
        } satisfies PluginUIRouteHost;
        const routeHost: PluginUIRouteHost = Object.freeze(
      ...
      try {
        ...
        if (closed) {
          ...
          const snapshot: PluginUIRouteSnapshot = Object.freeze(
    ...
    return Object.freeze<PluginUIRouteService>({
      register: (nameValue, definitionValue) => {
        ...
        if (previous === undefined && this.#registrations.size >= MAX_PLUGIN_UI_ROUTES_PER_GENERATION) {
          throw new RangeError(
            `Plugin UI routes are limited to ${MAX_PLUGIN_UI_ROUTES_PER_GENERATION} registrations per generation`,
          );
        }
        const current = this.#current;
        ...
        return Object.freeze<PluginUIRouteRegistration>({
      ...
      open: (nameValue, options) => {
        ...
        if (state === undefined) throw new Error(`Plugin UI route is not registered: ${name}`);
