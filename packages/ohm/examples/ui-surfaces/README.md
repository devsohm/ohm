...
```text
ohm plugins install ./packages/ohm/examples/ui-surfaces
```
...
the plugin generation and never take raw terminal ownership.
...
an unhandled Escape returns to the transcript. Only one plugin route is
...
At session start, the plugin wraps the active autocomplete provider with
...
The plugin checks `context.ui.capabilities` before using each surface. Rich
