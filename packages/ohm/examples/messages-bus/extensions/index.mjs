...
export default function activate(ohm) {
  ...
  ohm.services.register("example.messages", {
    publish(value) { ohm.events.emit("example-message", value); },
  });
  ohm.onDispose(stop);
  ...
  ohm.registerCommand("example-message", {
    ...
    async handler(args) {
      ohm.services.get("example.messages")?.publish(args.trim() || "Example message");
    },
  });
}
