...
```text
ohm plugins install ./packages/ohm/examples/project-trust
```

An approval lasts only for the current invocation because the example does not
set `remember`.

A trust plugin must come from an already trusted user or explicit source.
A project cannot authorize its own plugin code.
